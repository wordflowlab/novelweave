...
- When NovelWeave needs to make precise changes to existing code without rewriting entire files.
...
- Validates file access against `.novelweaveignore` rules.
...
2.  **NovelWeaveIgnore Check**: Validates if the target file path is allowed by `.novelweaveignore` rules.
...
NovelWeave uses this strategy for applying diffs:
