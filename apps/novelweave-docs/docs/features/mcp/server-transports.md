...
Model Context Protocol (MCP) supports two primary transport mechanisms for communication between NovelWeave and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (NovelWeave) spawns an MCP server as a child process
...
- **Locality**: Runs on the same machine as NovelWeave
...
- Single-client scenarios (one NovelWeave instance per server)
...
1. The client (NovelWeave) connects to the server's SSE endpoint via HTTP GET request
...
- **Remote Access**: Can be hosted on a different machine from NovelWeave
...
STDIO servers run locally on the same machine as NovelWeave, which has several important implications:
...
- **Execution**: Starts and stops with NovelWeave (child process lifecycle)
...
- Start when needed by NovelWeave
- Not require network configuration
- Need to be installed alongside NovelWeave or via a package manager
...
## Configuring Transports in NovelWeave

For detailed information on configuring STDIO and SSE transports in NovelWeave, including example configurations, see the [Understanding Transport Types](/features/mcp/using-mcp-in-novelweave#understanding-transport-types) section in the Using MCP in NovelWeave guide.
