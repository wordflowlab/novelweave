...
<figure style={{ float: 'right', width: '40%', maxWidth: '350px', margin: '0 0 10px 20px' }}>
  ...
  <figcaption style={{ fontSize: '0.9rem', color: '#666', marginTop: '8px', textAlign: 'center' }}>
    NovelWeave works more efficiently with Memory Bank enabled, instantly understanding project context and technologies.
  </figcaption>
</figure>
...
Memory Bank is a system of structured documentation that enables NovelWeave to **better understand your project** and **maintain context across coding sessions**. It transforms your AI assistant from a stateless tool into a persistent development partner with perfect recall of your project details. NovelWeave automatically reads your Memory Bank files to rebuild its understanding of your project whenever you start a new session.

When Memory Bank is active, NovelWeave begins each task with `[Memory Bank: Active]` and a brief summary of your project context, ensuring consistent understanding without repetitive explanations.

## Key Benefits

- **Language Agnostic**: Functions with any programming language or framework
- **Efficient Project Understanding**: Helps NovelWeave understand the purpose and tech stack of a project
- **Context Preservation**: Maintain project knowledge across sessions without needing to scan files in every new session
- **Faster Startup**: NovelWeave immediately comprehends your project context when you begin a new session
- **Self-Documenting Projects**: Create valuable documentation as a byproduct

## How Memory Bank Works

Memory Bank is built on NovelWeave's [Custom Rules](/advanced-usage/custom-rules) feature, providing a specialized framework for project documentation. Memory Bank files are standard markdown files stored in `.novelweave/rules/memory-bank` folder within your project repository. They're not hidden or proprietary - they're regular documentation files that both you and NovelWeave can access.

At the start of every task, NovelWeave reads all Memory Bank files to build a comprehensive understanding of your project. This happens automatically without requiring any action from you. NovelWeave then indicates successful Memory Bank activation with `[Memory Bank: Active]` at the beginning of its response, followed by a brief summary of its understanding of your project.
...
Note: NovelWeave will not edit this file directly but may suggest improvements if it identifies ways to enhance your project brief.
...
1. Create a `.novelweave/rules/memory-bank/` folder in your project
2. Write a basic project brief in `.novelweave/rules/memory-bank/brief.md`
3. Create a file `.novelweave/rules/memory-bank-instructions.md` and paste there [this document](pathname:///downloads/memory-bank.md)
4. Switch to `Architect` mode
5. Check if a best available AI model is selected, don't use "lightweight" models
6. Ask NovelWeave to "initialize memory bank"
7. Wait for NovelWeave to analyze your project and initialize the Memory Bank files
...
- NovelWeave will help fill in gaps and ask questions
...
The initialization step is critically important as it establishes the foundation for all future interactions with your project. When you request initialization with the command `initialize memory bank`, NovelWeave will:
...
2. Create comprehensive memory bank files in the `.novelweave/rules/memory-bank` folder
...
1. NovelWeave discovers new project patterns
2. After implementing significant changes
3. When you explicitly request with `update memory bank`
4. When context needs clarification

To execute a Memory Bank update, NovelWeave will:
...
You can direct NovelWeave to focus on specific information sources by using commands like `update memory bank using information from @/Makefile`.

#### Regular Task Execution

At the beginning of every task, NovelWeave:
...
At the end of a task, NovelWeave may suggest updating the memory bank if significant changes were made, using the phrase: "Would you like me to update memory bank to reflect these changes?"
...
To document a task, use the command `add task` or `store this as a task`. NovelWeave will:
...
When starting a new task, NovelWeave will check if it matches any documented tasks and follow the established workflow to ensure no steps are missed.
...
NovelWeave uses status indicators to clearly communicate Memory Bank status:
...
These indicators appear at the beginning of NovelWeave's responses, providing immediate confirmation of Memory Bank status.
...
As you work with NovelWeave, your context window will eventually fill up. When you notice responses slowing down or references becoming less accurate:

1. Ask NovelWeave to "update memory bank" to document the current state
2. Start a new conversation/task
3. NovelWeave will automatically access your Memory Bank in the new conversation

This process ensures continuity across multiple sessions without losing important context.

## Handling Inconsistencies

If NovelWeave detects inconsistencies between memory bank files:
...
This ensures that even with imperfect documentation, NovelWeave can still function effectively.

## Frequently Asked Questions

### Where are the memory bank files stored?

The Memory Bank files are regular markdown files stored in your project repository, typically in a `.novelweave/rules/memory-bank/` folder. They're not hidden system files - they're designed to be part of your project documentation.
...
Yes! While NovelWeave manages most of the files, you can edit any of them manually. The `brief.md` file is specifically designed to be maintained by you. Manual edits to other files will be respected by NovelWeave.

### What happens if memory bank files are missing?

If memory bank files are missing, NovelWeave will indicate this with `[Memory Bank: Missing]` at the beginning of its response and suggest initializing the memory bank.
...
Yes! Each project has its own Memory Bank in its `.novelweave/rules/memory-bank/` folder. NovelWeave will automatically use the correct Memory Bank for each project.
...
- Let NovelWeave help create the initial structure
...
The Memory Bank is NovelWeave's only link to previous work. Its effectiveness depends entirely on maintaining clear, accurate documentation and confirming context preservation in every interaction. When you see `[Memory Bank: Active]` at the beginning of a response, you can be confident that NovelWeave has a comprehensive understanding of your project.
