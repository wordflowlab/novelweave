...
export class ExtensionContext {
	...
	constructor(extensionPath: string, workspacePath: string) {
		...
		NovelWeavePaths.initializeWorkspace(workspacePath)

		const globalStoragePath = NovelWeavePaths.getGlobalStorageDir()
		const workspaceStoragePath = NovelWeavePaths.getWorkspaceStorageDir(workspacePath)
		const logPath = NovelWeavePaths.getLogsDir()
...
export class MockWorkspaceConfiguration implements WorkspaceConfiguration {
	...
	constructor(section?: string, context?: ExtensionContext) {
		...
		if (context) {
			...
			const globalStoragePath = NovelWeavePaths.getGlobalStorageDir()
			const workspaceStoragePath = NovelWeavePaths.getWorkspaceStorageDir(process.cwd())
	...
	return {
		...
		extensions: {
			...
			getExtension: (extensionId: string) => {
				...
				if (extensionId === "novelweave.novelweave") {
					return {
						id: extensionId,
