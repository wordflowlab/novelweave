...
import { NovelWeavePaths } from "../../utils/paths.js"
...
export interface UserIdentity {
	/** Persistent CLI user ID (UUID stored in ~/.novelweave/cli/identity) */
	cliUserId: string

	/** Machine identifier (OS-level) */
	machineId: string

	/** Novelweave user ID (from authentication token) */
	novelweaveUserId?: string
...
export class IdentityManager {
	...
	private constructor() {
		this.identityFilePath = path.join(NovelWeavePaths.getNovelWeaveDir(), "identity.json")
	}
	...
	/**
	 * Update Novelweave user ID from authentication token
	 */
	public async updateNovelweaveUserId(novelweaveToken: string): Promise<void> {
		if (!this.identity) {
			logs.warn("Cannot update Novelweave user ID: identity not initialized", "IdentityManager")
			return
		}

		try {
			// Fetch user profile from Novelweave API
			const response = await fetch("https://api.novelweave.ai/api/profile", {
				headers: {
			...
			if (data?.user?.email) {
				this.identity.novelweaveUserId = data.user.email
				logs.debug("Novelweave user ID updated", "IdentityManager", {
			...
			logs.warn("Failed to update Novelweave user ID", "IdentityManager", { error })
			// Clear Novelweave user ID on error
			if (this.identity.novelweaveUserId) {
				delete this.identity.novelweaveUserId
			}
		}
	}

	/**
	 * Clear Novelweave user ID (on logout)
	 */
	public clearNovelweaveUserId(): void {
		if (this.identity && this.identity.novelweaveUserId) {
			delete this.identity.novelweaveUserId
			logs.debug("Novelweave user ID cleared", "IdentityManager")
		}
	}
	...
	/**
	 * Get distinct ID for PostHog (prioritize Novelweave user ID)
	 */
	public getDistinctId(): string {
		...
		// Use Novelweave user ID if available, otherwise use CLI user ID
		return this.identity.novelweaveUserId || this.identity.cliUserId
	}
