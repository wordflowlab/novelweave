...
vi.mock("../../constants/telemetry.js", async () => {
	return {
		NOVELWEAVE_POSTHOG_API_KEY: "test-posthog-api-key",
	}
})
...
describe("TelemetryService", () => {
	...
	it("should initialize with config", async () => {
		...
		// In test environment without NOVELWEAVE_POSTHOG_API_KEY, telemetry will be disabled
...
describe("IdentityManager", () => {
	...
	it("should clear Novelweave user ID", async () => {
		await identityManager.initialize()
		identityManager.clearNovelweaveUserId()

		const identity = identityManager.getIdentity()
		expect(identity?.novelweaveUserId).toBeUndefined()
	})
})
