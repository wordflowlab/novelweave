...
import { NOVELWEAVE_POSTHOG_API_KEY } from "../../constants/telemetry.js"
...
export class TelemetryService {
	...
	public async initialize(
		...
		try {
			...
			const apiKey = NOVELWEAVE_POSTHOG_API_KEY
			if (!apiKey) {
				logs.warn("NOVELWEAVE_POSTHOG_API_KEY not set, telemetry disabled", "TelemetryService")
				this.isInitialized = true
				return
			}
			...
			// Update Novelweave user ID if token is available
			const provider = config.providers.find((p) => p.id === config.provider)
			if (provider && provider.novelweaveToken) {
				await identityManager.updateNovelweaveUserId(provider.novelweaveToken)
			}
	...
	/**
	 * Update Novelweave user ID
	 */
	public async updateNovelweaveUserId(novelweaveToken: string): Promise<void> {
		if (this.client) {
			await this.client.updateNovelweaveUserId(novelweaveToken)
		}
	}

	/**
	 * Clear Novelweave user ID
	 */
	public clearNovelweaveUserId(): void {
		if (this.client) {
			this.client.clearNovelweaveUserId()
		}
	}
