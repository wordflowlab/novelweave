...
describe("validateProviderConfig", () => {
	it("should always return valid since schema handles all validations", () => {
		...
		const provider: ProviderConfig = {
			id: "test-novelweave",
			provider: "novelweave",
			novelweaveToken: "valid-token-123",
			novelweaveModel: "claude-3-5-sonnet",
		}
...
describe("validateConfig", () => {
		...
		const config: CLIConfig = {
			...
			providers: [
				...
				},
				{
					id: "provider2",
					provider: "novelweave",
					novelweaveToken: "", // Empty is OK for non-selected provider
					novelweaveModel: "claude-3-5-sonnet",
				},
			],
		}
	...
	it("should reject config when selected provider has empty credentials (schema validation)", async () => {
			...
			providers: [
				{
					id: "provider1",
					provider: "novelweave",
					novelweaveToken: "", // Empty is NOT OK - schema minLength will catch this
					novelweaveModel: "claude-3-5-sonnet",
				},
			],
		}
		...
		expect(result.errors?.some((e) => e.includes("novelweaveToken"))).toBe(true)
	})
	...
	it("should return errors for multiple invalid providers (schema validation)", async () => {
		const config: CLIConfig = {
			...
			providers: [
				...
				},
				{
					id: "provider2",
					provider: "novelweave",
					novelweaveToken: "short", // Invalid - schema will catch
					novelweaveModel: "claude-3-5-sonnet",
				},
			],
		}
		...
		expect(result.errors?.some((e) => e.includes("apiKey") || e.includes("novelweaveToken"))).toBe(true)
	})
