...
describe("Provider Merging", () => {
	...
	it("should merge provider field from defaults when missing in loaded config", async () => {
			...
			providers: [
				{
					...
					novelweaveToken: "test-token-1234567890",
					novelweaveModel: "anthropic/claude-sonnet-4.5",
				},
			],
			theme: "dark",
		}
		...
		expect(result.config.providers[0].provider).toBe("novelweave")
		expect(result.config.providers[0].id).toBe("default")
		expect(result.config.providers[0].novelweaveToken).toBe("test-token-1234567890")
		expect(result.config.providers[0].novelweaveModel).toBe("anthropic/claude-sonnet-4.5")
