...
export function validateProviderConfig(provider: ProviderConfig, isSelected: boolean = false): ValidationResult {
	...
	switch (provider.provider) {
		case "novelweave":
			if (!provider.novelweaveToken || provider.novelweaveToken.length === 0) {
				errors.push("novelweaveToken is required and cannot be empty for selected provider")
			}
			if (!provider.novelweaveModel || provider.novelweaveModel.length === 0) {
				errors.push("novelweaveModel is required and cannot be empty for selected provider")
			}
			break
