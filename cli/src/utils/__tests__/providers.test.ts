...
describe("getSelectedModelId", () => {
	it("should return 'unknown' when apiConfiguration is undefined", () => {
		const result = getSelectedModelId("novelweave", undefined)
		expect(result).toBe("unknown")
	})

	it("should return 'unknown' when provider is unknown", () => {
		const result = getSelectedModelId("unknown", { novelweaveModel: "test-model" })
		expect(result).toBe("unknown")
	})

	it("should return 'unknown' when provider is empty", () => {
		const result = getSelectedModelId("", { novelweaveModel: "test-model" })
		expect(result).toBe("unknown")
	})

	it("should return correct model for novelweave provider", () => {
		const apiConfig = { novelweaveModel: "anthropic/claude-sonnet-4" }
		const result = getSelectedModelId("novelweave", apiConfig)
	...
	it("should handle mixed case provider names", () => {
		const apiConfig = { novelweaveModel: "test-model" }
		const result = getSelectedModelId("NovelWeave" as any, apiConfig)
