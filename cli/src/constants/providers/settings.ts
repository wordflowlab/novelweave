...
export const isOptionalField = (field: string): boolean => {
	...
	// Fallback logic for fields not in registry
	return field.includes("BaseUrl") || field === "novelweaveOrganizationId"
}
...
export const getProviderSettings = (provider: ProviderName, config: ProviderSettings): ProviderSettingConfig[] => {
	switch (provider) {
		case "novelweave":
			return [
				createFieldConfig("novelweaveToken", config),
				createFieldConfig("novelweaveOrganizationId", config, "personal"),
				createFieldConfig("novelweaveModel", config, "anthropic/claude-sonnet-4"),
			]
...
export const PROVIDER_DEFAULT_MODELS: Record<ProviderName, string> = {
	novelweave: "anthropic/claude-sonnet-4",
