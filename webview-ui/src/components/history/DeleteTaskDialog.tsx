...
export const DeleteTaskDialog = ({ taskId, ...props }: DeleteTaskDialogProps) => {
	...
	const { data: tasks } = useTaskWithId([taskId]) // novelweave_change

	const { onOpenChange } = props

	const task = tasks?.find((t) => t.id === taskId) // novelweave_change
	const isFavorited = task?.isFavorited // novelweave_change
