...
import { useTaskWithId } from "@/novelweave/hooks/useTaskHistory"
...
export const BatchDeleteTaskDialog = ({ taskIds, ...props }: BatchDeleteTaskDialogProps) => {
	...
	const { data: tasks } = useTaskWithId(taskIds) // novelweave_change
	const { onOpenChange } = props

	const favoritedTasks = tasks?.filter((task) => taskIds.includes(task.id) && task.isFavorited) ?? [] // novelweave_change
	const hasFavoritedTasks = favoritedTasks.length > 0 // novelweave_change
