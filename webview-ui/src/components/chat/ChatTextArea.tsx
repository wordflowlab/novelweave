...
import { DropdownOptionType, Button, StandardTooltip } from "@/components/ui" // novelweave_change
...
import {
	...
	Pin,
	Check,
	// Image, // novelweave_change
	WandSparkles,
	SendHorizontal,
	Paperclip, // novelweave_change
	MessageSquareX,
} from "lucide-react"
...
// novelweave_change start: pull slash commands from Cline
...
export const ChatTextArea = forwardRef<HTMLTextAreaElement, ChatTextAreaProps>(
	(
		...
		const {
			...
			localWorkflows, // novelweave_change
			globalWorkflows, // novelweave_change
		...
		// novelweave_change begin: remove button from chat when it gets to small
		...
		const [imageWarning, setImageWarning] = useState<string | null>(null) // novelweave_change
		...
		// novelweave_change start: Image warning handlers
		...
		// novelweave_change end: Image warning handlers

		// novelweave_change start: Clear images if unsupported
		...
		useEffect(() => {
			if (!prevShouldDisableImages.current && shouldDisableImages && selectedImages.length > 0) {
				setSelectedImages([])
				showImageWarning("novelweave:imageWarnings.imagesRemovedNoSupport")
			}
		...
		// novelweave_change end: Clear images if unsupported
		...
		const handleInputChange = useCallback(
			(e: React.ChangeEvent<HTMLTextAreaElement>) => {
				...
				if (showMenu) {
					// novelweave_change start - check lastAtIndex before handling slash commands
					const lastAtIndex = newValue.lastIndexOf("@", newCursorPosition - 1)

					// if (newValue.startsWith("/")) { ⚠️ novelweave_change added lastAtIndex check
		...
		const handleBlur = useCallback(() => {
			...
			if (!isMouseDownOnMenu) {
				setShowContextMenu(false)
				setShowSlashCommandsMenu(false) // novelweave_change: pull slash commands from Cline
			}
		...
		const handlePaste = useCallback(
			async (e: React.ClipboardEvent) => {
				...
				// novelweave_change start: Image validation with warning messages
				if (imageItems.length > 0) {
					e.preventDefault()

					if (shouldDisableImages) {
						showImageWarning(`novelweave:imageWarnings.modelNoImageSupport`)
						return
					}
					if (selectedImages.length >= MAX_IMAGES_PER_MESSAGE) {
						showImageWarning(`novelweave:imageWarnings.maxImagesReached`)
						return
					}
					// novelweave_change end: Image validation with warning messages
			...
			[
				...
				inputValue,
				t,
				selectedImages.length, // novelweave_change - added selectedImages.length
				showImageWarning, // novelweave_change - added showImageWarning
			],
		)
		...
		const handleDrop = useCallback(
			async (e: React.DragEvent<HTMLDivElement>) => {
				...
				if (files.length > 0) {
					...
					// novelweave_change start: Image validation with warning messages for drag and drop
					if (imageFiles.length > 0) {
						...
						// novelweave_change end: Image validation with warning messages for drag and drop
		...
		const renderTextAreaSection = () => (
			<div
				...
				<DynamicTextArea
					...
					// novelweave_change: combine placeholderText and placeholderBottomText here
					...
					className={cn(
						...
						"scrollbar-none",
						"scrollbar-hide",
						"pb-16", // novelweave_change: Increased padding to prevent overlap with control bar
					)}
					onScroll={() => updateHighlights()}
				/>
				{/* novelweave_change {Transparent overlay at bottom of textArea to avoid text overlap } */}
				<div
				...
				{/* novelweave_change: position tweaked */}
				...
				{/* novelweave_change: position tweaked, rtl support */}
				...
				{!inputValue && (
					<div
						...
						{/* novelweave_change {placeholderBottomText} */}
					</div>
				)}
			</div>
		)
			...
			<div
				...
				<div className="relative">
					<div
						...
						{/* novelweave_change start: ImageWarningBanner integration */}
						<ImageWarningBanner
							messageKey={imageWarning ?? ""}
							onDismiss={dismissImageWarning}
							isVisible={!!imageWarning}
						/>
						{/* novelweave_change end: ImageWarningBanner integration */}
						{/* novelweave_change start: pull slash commands from Cline */}
						{showSlashCommandsMenu && (
						...
						{/* novelweave_change end: pull slash commands from Cline */}
						{showContextMenu && (
							<div
						...
						<div
							...
							{/* novelweave_change: hidden on small containerWidth
				...
				{selectedImages.length > 0 && (
					<Thumbnails
						...
						style={{
							left: "16px",
							zIndex: 2,
							marginTop: "14px", // novelweave_change
							marginBottom: 0,
						}}
					/>
				)}
