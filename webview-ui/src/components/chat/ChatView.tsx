...
// import RooHero from "@src/components/welcome/RooHero" // novelweave_change: unused
// import RooTips from "@src/components/welcome/RooTips" // novelweave_change: unused
import { StandardTooltip } from "@src/components/ui"
import { useAutoApprovalState } from "@src/hooks/useAutoApprovalState"
import { useAutoApprovalToggles } from "@src/hooks/useAutoApprovalToggles"
// import { CloudUpsellDialog } from "@src/components/cloud/CloudUpsellDialog" // novelweave_change: unused

import TelemetryBanner from "../common/TelemetryBanner" // novelweave_change: deactivated for now
// import VersionIndicator from "../common/VersionIndicator" // novelweave_change: unused
import { OrganizationSelector } from "../novelweave/common/OrganizationSelector"
// import { useTaskSearch } from "../history/useTaskSearch" // novelweave_change: unused
...
import { showSystemNotification } from "@/novelweave/helpers" // novelweave_change
// import ProfileViolationWarning from "./ProfileViolationWarning" novelweave_change: unused
import { CheckpointWarning } from "./CheckpointWarning"
import { IdeaSuggestionsBox } from "../novelweave/chat/IdeaSuggestionsBox" // novelweave_change
import { NovelweaveNotifications } from "../novelweave/NovelweaveNotifications" // novelweave_change
import { QueuedMessages } from "./QueuedMessages"
import { buildDocLink } from "@/utils/docLinks"
// import DismissibleUpsell from "../common/DismissibleUpsell" // novelweave_change: unused
// import { useCloudUpsell } from "@src/hooks/useCloudUpsell" // novelweave_change: unused
// import { Cloud } from "lucide-react" // novelweave_change: unused
...
export interface ChatViewRef {
	acceptInput: () => void
	focusInput: () => void // novelweave_change
}
...
const ChatViewComponent: React.ForwardRefRenderFunction<ChatViewRef, ChatViewProps> = (
	...
	const {
		...
		currentTaskItem,
		currentTaskTodos,
		taskHistoryFullLength, // novelweave_change
		taskHistoryVersion, // novelweave_change
		apiConfiguration,
		...
		enableCheckpoints, // novelweave_change
		alwaysAllowSubtasks,
		...
		soundEnabled,
		soundVolume,
		// cloudIsAuthenticated, // novelweave_change
		messageQueue = [],
	} = useExtensionState()
	...
	// const { tasks } = useTaskSearch() // novelweave_change
	...
	// novelweave_change start: unused
	// const {
	// 	isOpen: isUpsellOpen,
	// 	openUpsell,
	// 	closeUpsell,
					...
					switch (lastMessage.ask) {
						...
						case "tool":
							if (!isAutoApproved(lastMessage) && !isPartial) {
								playSound("notification")
								showSystemNotification(t("novelweave:notifications.toolRequest")) // novelweave_change
							}
							setSendingDisabled(isPartial)
							setClineAsk("tool")
						...
						case "browser_action_launch":
							if (!isAutoApproved(lastMessage) && !isPartial) {
								playSound("notification")
								showSystemNotification(t("novelweave:notifications.browserAction")) // novelweave_change
							}
							setSendingDisabled(isPartial)
						...
						case "command":
							if (!isAutoApproved(lastMessage) && !isPartial) {
								playSound("notification")
								showSystemNotification(t("novelweave:notifications.command")) // novelweave_change
							}
							setSendingDisabled(isPartial)
							setClineAsk("command")
							setEnableButtons(!isPartial)
						...
						case "condense":
							...
							setEnableButtons(!isPartial)
							setPrimaryButtonText(t("novelweave:chat.condense.condenseConversation"))
							setSecondaryButtonText(undefined)
							break
						// novelweave_change end
					}
	...
	const handlePrimaryButtonClick = useCallback(
		...
		[clineAsk, startNewTask, lastMessage?.text], // novelweave_change: add lastMessage?.text
	)
	...
	const handleTaskCloseButtonClick = useCallback(() => startNewTask(), [startNewTask]) // novelweave_change
	...
	const isAllowedCommand = useCallback(
		(message: ClineMessage | undefined): boolean => {
			// novelweave_change start wrap in try/catch
	...
	const groupedMessages = useMemo(() => {
		...
		visibleMessages.forEach((message: ClineMessage) => {
			// novelweave_change start: upstream pr https://github.com/RooCodeInc/Roo-Code/pull/5452
	...
	const itemContent = useCallback(
		(index: number, messageOrGroup: ClineMessage | ClineMessage[]) => {
			...
			return (
				<ChatRow
					...
					highlighted={highlightedMessageIndex === index} // novelweave_change: add highlight prop
					enableCheckpoints={enableCheckpoints} // novelweave_change
		...
		[
			...
			highlightedMessageIndex, // novelweave_change: add highlightedMessageIndex
			enableCheckpoints, // novelweave_change
	...
	useEffect(() => {
		...
		window.addEventListener("wheel", handleWheel, { passive: true }) // novelweave_change
		return () => {
			window.removeEventListener("keydown", handleKeyDown)
			window.removeEventListener("wheel", handleWheel) // novelweave_change
		}
	}, [handleKeyDown, handleWheel]) // novelweave_change
	...
	const showTelemetryBanner = telemetrySetting === "unset" // novelweave_change
		...
		<div
			...
			{task ? (
				...
				<div className="flex-1 min-h-0 overflow-y-auto flex flex-col gap-4 relative">
					...
					{/* novelweave_change start: changed the classes to support notifications */}
					<div className="w-full h-full flex flex-col gap-4 px-3.5 transition-all duration-300">
						{/* novelweave_change end */}
						{/* Version indicator in top-right corner - only on welcome screen */}
						{/* novelweave_change: do not show */}
						{/* <VersionIndicator
						...
						<div className="flex flex-grow flex-col justify-center gap-4">
							...
							{taskHistoryFullLength === 0 && <IdeaSuggestionsBox />} {/* novelweave_change */}
							{/*<div className="mb-2.5">
								{cloudIsAuthenticated || taskHistory.length < 4 ? <RooTips /> : <RooCloudCTA />}
							</div> novelweave_change: do not show */}
			...
			*/}
			{/* novelweave_change: added settings toggle for this */}
				...
				<>
					<div className="grow flex" ref={scrollContainerRef}>
						<Virtuoso
							...
							increaseViewportBy={{ top: 400, bottom: 400 }} // novelweave_change: use more modest numbers to see if they reduce gray screen incidence
			...
			/>
			{/* novelweave_change: added settings toggle the profile and model selection */}
			<BottomControls showApiConfig />
			{/* novelweave_change: end */}

			{/* novelweave_change: disable {isProfileDisabled && (
				<div className="px-3">
