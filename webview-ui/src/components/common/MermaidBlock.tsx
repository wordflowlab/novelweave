...
import { MermaidSyntaxFixer } from "@src/services/mermaidSyntaxFixer" // novelweave_change
import CodeBlock from "../novelweave/common/CodeBlock" // novelweave_change
import { MermaidButton } from "@/components/common/MermaidButton"
import { MermaidFixButton } from "@/components/common/MermaidFixButton" // novelweave_change
...
// novelweave_change next line rename to originalCode to keep the difference with Roo smaller
export default function MermaidBlock({ code: originalCode }: MermaidBlockProps) {
	...
	useEffect(() => {
		...
		setIsFixing(false)
		// novelweave_change end
	}, [originalCode]) // novelweave_change originalCode instead of code
	...
	useDebounceEffect(
		...
		500, // Delay 500ms
		[code, isFixing, originalCode, t], // Dependencies for scheduling // novelweave_change added isFixing, originalCode and t
	)
	...
	return (
		<MermaidBlockContainer>
			{isLoading && (
				<LoadingMessage>
					{isFixing /* novelweave_change */ ? t("common:mermaid.fixing_syntax") : t("common:mermaid.loading")}
				</LoadingMessage>
			)}
			...
			{error ? (
				...
				<MermaidButton containerRef={containerRef} code={code} isLoading={isLoading} svgToPng={svgToPng}>
					{/* novelweave_change start switched from ref to dangerouslySetInnerHTML */}
					<SvgContainer
						onClick={handleClick}
						ref={containerRef}
						$isLoading={isLoading}
