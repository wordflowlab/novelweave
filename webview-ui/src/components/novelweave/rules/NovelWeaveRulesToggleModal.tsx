					...
					<Tooltip open={isVisible ? false : undefined}>
						<TooltipTrigger asChild>
							<BottomButton
								...
								ariaLabel={t("novelweave:rules.ariaLabel")}
								onClick={() => setIsVisible(!isVisible)}
							/>
						</TooltipTrigger>
						<TooltipContent>{t("novelweave:rules.tooltip")}</TooltipContent>
					</Tooltip>
				</TooltipProvider>
			</div>
			...
			{isVisible && (
				<div
					...
					<div
						...
						<div
							...
							<StyledTabButton $isActive={currentView === "rule"} onClick={() => setCurrentView("rule")}>
								{t("novelweave:rules.tabs.rules")}
							</StyledTabButton>
							<StyledTabButton
								$isActive={currentView === "workflow"}
								onClick={() => setCurrentView("workflow")}>
								{t("novelweave:rules.tabs.workflows")}
							</StyledTabButton>
						</div>
					</div>
					...
					<div className="text-xs text-[var(--vscode-descriptionForeground)] mb-4">
						{currentView === "rule" ? (
							<DescriptionWithLink
								href="https://novelweave.ai/docs/advanced-usage/custom-rules"
								linkText={t("novelweave:docs")}>
								{t("novelweave:rules.description.rules")}
							</DescriptionWithLink>
						) : (
							<DescriptionWithLink
								href="https://novelweave.ai/docs/features/slash-commands/workflows"
								linkText={t("novelweave:docs")}>
								{t("novelweave:rules.description.workflows")}{" "}
								<span className="text-[var(--vscode-foreground)] font-bold">/workflow-name</span>{" "}
								{t("novelweave:rules.description.workflowsInChat")}
							</DescriptionWithLink>
						)}
					</div>
