...
export const OrganizationSelector = ({ className, showLabel = false }: { className?: string; showLabel?: boolean }) => {
	...
	const selectedOrg = organizations.find((o) => o.id === apiConfiguration?.novelweaveOrganizationId)
	...
	useEffect(() => {
		if (!apiConfiguration?.novelweaveToken) return
	...
	const setSelectedOrganization = (organization: UserOrganizationWithApiKey | null) => {
		if (organization === null) {
			...
			vscode.postMessage({
				...
				values: {
					apiKey: apiConfiguration?.novelweaveToken,
				},
			})
		} else {
			vscode.postMessage({
				...
				apiConfiguration: {
					...apiConfiguration,
					novelweaveOrganizationId: organization.id,
				},
			})
			vscode.postMessage({
				type: "fetchBalanceDataRequest",
	...
	return (
		<div className={className}>
			{showLabel && (
				<div>
					<label className="block font-medium mb-2">{t("novelweave:profile.organization")}</label>
				</div>
			)}
			<div className="relative" ref={containerRef}>
				<button
					...
					<span className="truncate">{selectedOrg ? selectedOrg.name : t("novelweave:profile.personal")}</span>
				...
				{isOpen && (
					<div className="absolute z-20 mt-1 right-0 w-max min-w-full max-h-60 overflow-auto rounded border border-[var(--vscode-dropdown-border)] bg-[var(--vscode-dropdown-background)] shadow">
						<div role="listbox" aria-label={t("novelweave:profile.organization")}>
							<button
								type="button"
								role="option"
								...
								<span className="truncate">{t("novelweave:profile.personal")}</span>
							</button>
