			...
			<table className="w-full text-sm">
				<thead>
					<tr className="border-b border-vscode-widget-border bg-vscode-editor-background">
						<th className={thClass}>{t("novelweave:settings.modelInfo.table.provider")}</th>
						<StandardTooltip content={t("novelweave:settings.modelInfo.table.context")}>
							<th className={thClass}>Context</th>
						</StandardTooltip>
						<StandardTooltip content={t("novelweave:settings.modelInfo.table.inputPrice")}>
							<th className={thClass}>Input</th>
						</StandardTooltip>
						<StandardTooltip content={t("novelweave:settings.modelInfo.table.outputPrice")}>
							<th className={thClass}>Output</th>
						</StandardTooltip>
						<StandardTooltip content={t("novelweave:settings.modelInfo.table.cacheReadPrice")}>
							<th className={thClass}>C Read</th>
						</StandardTooltip>
						<StandardTooltip content={t("novelweave:settings.modelInfo.table.cacheWritePrice")}>
...
export const NovelweaveModelInfoView = ({
	apiConfiguration,
	modelId,
	model,
	...
	return (
		<>
			...
			{apiConfiguration.apiProvider === "novelweave" && modelId.endsWith(":free") && (
			...
			<div className="text-sm text-vscode-descriptionForeground">
				...
				/>
				<div>
					<span className="font-medium">{t("novelweave:settings.modelInfo.contextWindow")}:</span>{" "}
			...
			{stats && stats.cost && stats.costPerRequest && model.inputPrice && model.outputPrice ? (
				<StandardTooltip content={t("novelweave:settings.modelInfo.averageNovelWeaveCost")}>
			...
			<Collapsible open={isPricingExpanded} onOpenChange={setIsPricingExpanded}>
				<CollapsibleTrigger className="flex items-center gap-1 w-full cursor-pointer hover:opacity-80 mb-2">
					...
					<span className="font-medium">{t("novelweave:settings.modelInfo.providerBreakdown")}</span>
				</CollapsibleTrigger>
