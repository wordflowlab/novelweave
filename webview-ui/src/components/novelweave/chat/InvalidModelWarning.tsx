...
import { isAlphaPeriodEndedError, isModelNotAllowedForTeamError } from "@roo/novelweave/errorUtils"
...
function warningText(innerMessage?: InnerMessage) {
	...
	if (isAlphaPeriodEndedError(innerMessage?.error)) {
		return i18next.t("novelweave:invalidModel.alphaPeriodEnded", { model: unavailableModel })
	}
	if (isModelNotAllowedForTeamError(innerMessage?.error)) {
		return i18next.t("novelweave:invalidModel.notAllowedForTeam", { model: unavailableModel })
	}
	return i18next.t("novelweave:invalidModel.modelUnavailable", { model: unavailableModel })
}
				...
				<>
					<VSCodeButton
						...
						{canChangeToDefaultModel
							? t("novelweave:invalidModel.continueWith", {
									model: providerModels[defaultModelId]?.displayName ?? defaultModelId,
								})
							: t("novelweave:invalidModel.continue")}
					</VSCodeButton>
