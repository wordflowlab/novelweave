...
export const MarketplaceItemCard: React.FC<MarketplaceItemCardProps> = ({ item, filters, setFilters, installed }) => {
	...
	const [installModalVersion, setInstallModalVersion] = useState(0) // novelweave_change
	...
	const handleInstallClick = () => {
		...
		setInstallModalVersion((prev) => prev + 1) // novelweave_change
	...
	return (
		<>
			...
			<MarketplaceInstallModal
				key={`install-modal-${item.id}-${installModalVersion}` /* novelweave_change */}
				item={item}
