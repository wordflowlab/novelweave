		...
		<div className="flex flex-col gap-1">
			<div className="flex justify-between items-center">
				<label className="block font-medium mb-1">
					{t("novelweave:settings.provider.providerRouting.title")}
				</label>
			</div>
			<div className="text-sm text-vscode-descriptionForeground">
				{t("novelweave:settings.provider.providerRouting.managedByOrganization")}
			</div>
		</div>
	)
}
			...
			<Select value={JSON.stringify(getProviderPreference(apiConfiguration))} onValueChange={onValueChange}>
				...
				<SelectContent>
					<ProviderSelectItem value={{ type: DEFAULT_VALUE_KEY }}>
						{t("novelweave:settings.provider.providerRouting.sorting.default")}
					</ProviderSelectItem>
					<ProviderSelectItem value={{ type: openRouterProviderSortSchema.Values.price }}>
						{t("novelweave:settings.provider.providerRouting.sorting.price")}
					</ProviderSelectItem>
					<ProviderSelectItem value={{ type: openRouterProviderSortSchema.Values.throughput }}>
						{t("novelweave:settings.provider.providerRouting.sorting.throughput")}
					</ProviderSelectItem>
					<ProviderSelectItem value={{ type: openRouterProviderSortSchema.Values.latency }}>
						{t("novelweave:settings.provider.providerRouting.sorting.latency")}
					</ProviderSelectItem>
					<SelectSeparator />
			...
			<Select
				...
				<SelectContent>
					<SelectItem value={DEFAULT_VALUE_KEY}>
						{apiConfiguration.apiProvider === "novelweave"
							? t("novelweave:settings.provider.providerRouting.dataCollection.allowFree")
							: t("novelweave:settings.provider.providerRouting.dataCollection.default")}
					</SelectItem>
					<SelectItem value={openRouterProviderDataCollectionSchema.Values.allow}>
						{t("novelweave:settings.provider.providerRouting.dataCollection.allow")}
					</SelectItem>
					<SelectItem value={openRouterProviderDataCollectionSchema.Values.deny}>
						{t("novelweave:settings.provider.providerRouting.dataCollection.deny")}
					</SelectItem>
					<SelectItem value={ZERO_DATA_RETENTION_KEY}>
						{t("novelweave:settings.provider.providerRouting.dataCollection.zdr")}
					</SelectItem>
				</SelectContent>
			</Select>
		</div>
