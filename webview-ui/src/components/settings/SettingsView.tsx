...
import {
	...
	Globe,
	Info,
	Server, // novelweave_change
	Bot, // novelweave_change
	MessageSquare,
	Monitor,
	LucideIcon,
	// SquareSlash, // novelweave_change
	// Glasses, // novelweave_change
} from "lucide-react"
...
import { DisplaySettings } from "./DisplaySettings" // novelweave_change
...
import McpView from "../novelweaveMcp/McpView" // novelweave_change
import deepEqual from "fast-deep-equal" // novelweave_change
import { GhostServiceSettingsView } from "../novelweave/settings/GhostServiceSettings" // novelweave_change
...
export const settingsTabTrigger =
	"whitespace-nowrap overflow-hidden min-w-0 h-12 px-4 py-3 box-border flex items-center border-l-2 border-transparent text-vscode-foreground opacity-70 hover:bg-vscode-list-hoverBackground data-[compact=true]:w-12 data-[compact=true]:p-4 cursor-pointer" // novelweave_change add cursor-pointer
export const settingsTabTriggerActive =
	"opacity-100 border-vscode-focusBorder bg-vscode-list-activeSelectionBackground hover:bg-vscode-list-activeSelectionBackground cursor-default" // novelweave_change add hover:bg-* and cursor-default
...
const sectionNames = [
	...
	"browser",
	"checkpoints",
	"ghost", // novelweave_change
	"display", // novelweave_change
	"notifications",
	"contextManagement",
	"terminal",
	"prompts",
...
type SectionName = (typeof sectionNames)[number] // novelweave_change
...
const SettingsView = forwardRef<SettingsViewRef, SettingsViewProps>(({ onDone, targetSection }, ref) => {
	...
	const {
		...
		listApiConfigMeta,
		uriScheme,
		kiloCodeWrapperProperties, // novelweave_change
		settingsImportedAt,
	} = extensionState
	...
	const { novelweaveToken, openRouterApiKey, glamaApiKey, requestyApiKey } = extensionState.apiConfiguration ?? {}
	useEffect(() => {
	...
	}, [novelweaveToken, openRouterApiKey, glamaApiKey, requestyApiKey])
	...
	const setNovelWeaveImageApiKey = useCallback((apiKey: string) => {
		setCachedState((prevState) => {
			setChangeDetected(true)
	...
	const handleSubmit = () => {
		if (isSettingValid) {
			...
			vscode.postMessage({ type: "terminalCommandApiConfigId", text: terminalCommandApiConfigId || "" }) // novelweave_change
			...
			vscode.postMessage({ type: "showAutoApproveMenu", bool: showAutoApproveMenu }) // novelweave_change
			...
			vscode.postMessage({ type: "allowVeryLargeReads", bool: allowVeryLargeReads }) // novelweave_change
			...
			vscode.postMessage({ type: "showTaskTimeline", bool: showTaskTimeline }) // novelweave_change
			vscode.postMessage({ type: "showTimestamps", bool: showTimestamps }) // novelweave_change
			vscode.postMessage({ type: "hideCostBelowThreshold", value: hideCostBelowThreshold }) // novelweave_change
			...
			vscode.postMessage({ type: "systemNotificationsEnabled", bool: systemNotificationsEnabled }) // novelweave_change
			vscode.postMessage({ type: "ghostServiceSettings", values: ghostServiceSettings }) // novelweave_change
			vscode.postMessage({ type: "morphApiKey", text: morphApiKey }) // novelweave_change
			vscode.postMessage({ type: "fastApplyModel", text: fastApplyModel }) // novelweave_change: Fast Apply model selection
	...
	const sections: { id: SectionName; icon: LucideIcon }[] = useMemo(
		() => [
			...
			// { id: "slashCommands", icon: SquareSlash }, // novelweave_change: needs work to be re-introduced
			{ id: "browser", icon: SquareMousePointer },
			{ id: "checkpoints", icon: GitBranch },
			{ id: "display", icon: Monitor }, // novelweave_change
			...(kiloCodeWrapperProperties?.kiloCodeWrapped ? [] : [{ id: "ghost" as const, icon: Bot }]), // novelweave_change
			{ id: "notifications", icon: Bell },
			{ id: "contextManagement", icon: Database },
			{ id: "terminal", icon: SquareTerminal },
			{ id: "prompts", icon: MessageSquare },
			// { id: "ui", icon: Glasses }, // novelweave_change: we have our own display section
			{ id: "experimental", icon: FlaskConical },
			{ id: "language", icon: Globe },
			{ id: "mcp", icon: Server },
			{ id: "about", icon: Info },
		],
		[kiloCodeWrapperProperties?.kiloCodeWrapped], // novelweave_change
	)
	...
	}, [targetSection]) // novelweave_change
			...
			<div ref={containerRef} className={cn(settingsTabsContainer, isCompactMode && "narrow")}>
							...
							<TabTrigger
								...
								<div className={cn("flex items-center gap-2", isCompactMode && "justify-center")}>
									...
									<span className="tab-label">
										{id === "mcp"
											? t(`novelweave:settings.sections.mcp`)
											: id === "ghost"
												? t(`novelweave:ghost.title`)
				...
				<TabContent ref={contentRef} className="p-0 flex-1 overflow-auto">
					...
					{activeTab === "autoApprove" && (
						<AutoApproveSettings
							showAutoApproveMenu={showAutoApproveMenu} // novelweave_change
					...
					{/* novelweave_change start display section */}
					{activeTab === "display" && (
						<DisplaySettings
							reasoningBlockCollapsed={reasoningBlockCollapsed ?? true}
							showTaskTimeline={showTaskTimeline}
							showTimestamps={cachedState.showTimestamps} // novelweave_change
					...
					{/* novelweave_change end display section */}
					...
					{activeTab === "contextManagement" && (
						<ContextManagementSettings
							...
							allowVeryLargeReads={allowVeryLargeReads /* novelweave_change */}
					...
					{activeTab === "terminal" && (
						<TerminalSettings
							...
							terminalCommandApiConfigId={terminalCommandApiConfigId} // novelweave_change
							setCachedStateField={setCachedStateField}
						/>
					)}
					...
					{activeTab === "experimental" && (
						<ExperimentalSettings
							...
							currentProfileNovelweaveToken={apiConfiguration.novelweaveToken}
						/>
					)}
