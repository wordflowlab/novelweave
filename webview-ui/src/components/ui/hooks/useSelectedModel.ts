...
import { useExtensionState } from "@/context/ExtensionStateContext" // novelweave_change

// novelweave_change start
export const useModelProviders = (novelweaveDefaultModel: string, apiConfiguration?: ProviderSettings) => {
	const provider = apiConfiguration?.apiProvider
	return useOpenRouterModelProviders(
		provider === "novelweave"
			? (apiConfiguration?.novelweaveModel ?? novelweaveDefaultModel)
			: provider === "openrouter"
		...
		apiConfiguration?.novelweaveOrganizationId ?? "personal",
	)
}
// novelweave_change end
...
export const useSelectedModel = (apiConfiguration?: ProviderSettings) => {
	...
	const openRouterModelProviders = useModelProviders(novelweaveDefaultModel, apiConfiguration)
	// novelweave_change end
...
function getSelectedModel({
	...
	switch (provider) {
		...
		case "chutes": {
			...
			const info = routerModels.chutes[id] // novelweave_change
			return { id, info }
		}
		case "bedrock": {
		...
		case "novelweave": {
			...
			if (routerModels["novelweave-openrouter"] && apiConfiguration.novelweaveModel) {
				// Find the model in the fetched models
				const modelEntries = Object.entries(routerModels["novelweave-openrouter"])

				const selectedModelId = apiConfiguration.novelweaveModel.toLowerCase()
			...
			const invalidOrDefaultModel = apiConfiguration.novelweaveModel ?? novelweaveDefaultModel
			return {
				id: invalidOrDefaultModel,
				info: routerModels["novelweave-openrouter"][invalidOrDefaultModel],
			}
		}
		case "gemini-cli": {
