...
import { IconProps } from "@radix-ui/react-icons/dist/types" // novelweave_change
...
export interface DropdownOption {
	value: string
	label: string
	codicon?: string // novelweave_change
	description?: string // novelweave_change
	disabled?: boolean
...
export interface SelectDropdownProps {
	...
	disabled?: boolean
	initiallyOpen?: boolean // novelweave_change
	title?: string
	...
	triggerIcon?: React.ForwardRefExoticComponent<IconProps & React.RefAttributes<SVGSVGElement>> | boolean | undefined // novelweave_change
}
		...
		(
			{
				...
				onChange,
				disabled = false,
				initiallyOpen = false, // novelweave_change
				title = "",
				triggerClassName = "",
				contentClassName = "",
				itemClassName = "",
				...
				renderItem,
				disableSearch = false,
				triggerIcon = CaretUpIcon, // novelweave_change
			},
			ref,
		) => {
			const { t } = useTranslation()
			const [open, setOpen] = React.useState(initiallyOpen) // novelweave_change
				...
				<Popover open={open} onOpenChange={onOpenChange} data-testid="dropdown-root">
					...
					<PopoverContent
						...
						<div className="flex flex-col w-full">
							...
							{/* novelweave_change: different max height: max-h-82 */}
										...
										{groupedOptions.map((option, index) => {
											...
											return (
												<div
													...
													className={cn(
														"text-sm cursor-pointer flex items-center", // novelweave_change
														option.disabled
															? "opacity-50 cursor-not-allowed"
