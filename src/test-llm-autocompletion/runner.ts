...
function checkEnvironment() {
	const provider = process.env.LLM_PROVIDER || "novelweave"

	if (provider !== "novelweave") {
		console.error(`\n❌ Error: Only novelweave provider is supported. Got: ${provider}`)
		process.exit(1)
	}

	if (!process.env.NOVELWEAVE_API_KEY) {
		console.error(`\n❌ Error: NOVELWEAVE_API_KEY is not set`)
		console.log("\nPlease create a .env file with your API credentials.")
		console.log("Example: NOVELWEAVE_API_KEY=your-api-key-here\n")
		process.exit(1)
	}
}
