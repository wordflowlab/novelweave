...
export class GhostProvider {
	...
	public async promptCodeSuggestion() {
		...
		const userInput = await vscode.window.showInputBox({
			prompt: t("novelweave:ghost.input.title"),
			placeHolder: t("novelweave:ghost.input.placeholder"),
		})
		if (!userInput) {
			return
		}
	...
	private async updateGlobalContext() {
		...
		await vscode.commands.executeCommand("setContext", "novelweave.ghost.hasSuggestions", hasSuggestions)
		await vscode.commands.executeCommand("setContext", "novelweave.ghost.isProcessing", this.isProcessing)
		await vscode.commands.executeCommand(
			"setContext",
			"novelweave.ghost.enableQuickInlineTaskKeybinding",
			this.settings?.enableQuickInlineTaskKeybinding || false,
		)
		await vscode.commands.executeCommand(
			"setContext",
			"novelweave.ghost.enableSmartInlineTaskKeybinding",
	...
	public async showIncompatibilityExtensionPopup() {
		const message = t("novelweave:ghost.incompatibilityExtensionPopup.message")
		const disableCopilot = t("novelweave:ghost.incompatibilityExtensionPopup.disableCopilot")
		const disableInlineAssist = t("novelweave:ghost.incompatibilityExtensionPopup.disableInlineAssist")
		...
		if (response === disableCopilot) {
			...
			await vscode.commands.executeCommand<any>("novelweave.ghost.disable")
		}
	}
