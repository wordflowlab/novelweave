...
const NOVELWEAVE_DEFAULT_MODEL = "mistralai/codestral-2508"
const MISTRAL_DEFAULT_MODEL = "codestral-latest"

const SUPPORTED_DEFAULT_PROVIDERS = ["mistral", "novelweave", "openrouter"]
...
export class GhostModel {
	...
	public async reload(settings: GhostServiceSettings, providerSettingsManager: ProviderSettingsManager) {
		...
		if (selectedProfile) {
			...
			let modelDefinition = {}
			if (profileProvider === "novelweave") {
				modelDefinition = {
					novelweaveModel: NOVELWEAVE_DEFAULT_MODEL,
				}
			} else if (profileProvider === "openrouter") {
				modelDefinition = {
					openRouterModelId: NOVELWEAVE_DEFAULT_MODEL,
				}
