...
export class TerminalWelcomeService {
	...
	private handleTerminalOpened(terminal: vscode.Terminal): void {
		...
		this.tipShownThisSession = true // novelweave_change: Mark as shown for this session
		setTimeout(() => this.showWelcomeMessage(terminal), 500)
	}

	private async showWelcomeMessage(terminal: vscode.Terminal): Promise<void> {
		const shortcut = await getKeybindingForCommand("novelweave.generateTerminalCommand")
		const message = t("novelweave:terminalCommandGenerator.tipMessage", { shortcut })
