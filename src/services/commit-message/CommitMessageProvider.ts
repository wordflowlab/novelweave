...
export class CommitMessageProvider {
	...
	public async activate(): Promise<void> {
		this.outputChannel.appendLine(t("novelweave:commitMessage.activated"))

		try {
			await this.providerSettingsManager.initialize()
		} catch (error) {
			this.outputChannel.appendLine(t("novelweave:commitMessage.gitInitError", { error }))
		}

		// Register the command
		const disposable = vscode.commands.registerCommand(
			"novelweave.generateCommitMessage",
		...
		await vscode.window.withProgress(
			{
				...
				title: t("novelweave:commitMessage.generating"),
				cancellable: false,
			},
			async (progress) => {
				try {
					...
					if (changes.length === 0) {
						...
						if (changes.length > 0) {
							vscode.window.showInformationMessage(t("novelweave:commitMessage.generatingFromUnstaged"))
						} else {
							vscode.window.showInformationMessage(t("novelweave:commitMessage.noChanges"))
							return
						}
					}

					// Report initial progress after gathering changes (10% of total)
					progress.report({ increment: 10, message: t("novelweave:commitMessage.generating") })
					...
					const onDiffProgress = (percentage: number) => {
						...
						if (increment > 0) {
							progress.report({ increment, message: t("novelweave:commitMessage.generating") })
					...
					vscode.window.showErrorMessage(t("novelweave:commitMessage.generationFailed", { errorMessage }))
	...
	private async callAIForCommitMessageWithProgress(
		...
		const progressInterval = setInterval(() => {
			...
			progress.report({ increment: increment, message: t("novelweave:commitMessage.generating") })
