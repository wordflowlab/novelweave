...
import type {
	ApiHandlerCreateMessageMetadata, // novelweave_change
	SingleCompletionHandler,
} from "../index"
import { verifyFinishReason } from "./novelweave/verifyFinishReason"
...
import { isAnyRecognizedNovelWeaveError } from "../../shared/novelweave/errorUtils"
...
interface CompletionUsage {
	...
	total_tokens?: number
	cost?: number
	is_byok?: boolean // novelweave_change
	cost_details?: {
...
export class OpenRouterHandler extends BaseProvider implements SingleCompletionHandler {
	...
	// novelweave_change start property
	protected get providerName(): "OpenRouter" | "NovelWeave" {
	...
	override async *createMessage(
		...
		metadata?: ApiHandlerCreateMessageMetadata, // novelweave_change
		...
		const completionParams: OpenRouterChatCompletionParams = {
			...
			...this.getProviderParams(), // novelweave_change: original expression was moved into function
		...
		try {
			stream = await this.client.chat.completions.create(
				completionParams,
				this.customRequestOptions(metadata), // novelweave_change
			)
		} catch (error) {
			// novelweave_change start
			if (this.providerName == "NovelWeave" && isAnyRecognizedNovelWeaveError(error)) {
				throw error
			}
		...
		let inferenceProvider: string | undefined // novelweave_change
		...
		try {
			for await (const chunk of stream) {
				...
				verifyFinishReason(chunk.choices[0]) // novelweave_change
				const delta = chunk.choices[0]?.delta

				if (
					delta /* novelweave_change */ &&
					"reasoning" in delta &&
					delta.reasoning &&
	...
	async completePrompt(prompt: string) {
		...
		try {
			response = await this.client.chat.completions.create(
				completionParams,
				this.customRequestOptions(), // novelweave_change
			)
		} catch (error) {
			throw handleOpenAIError(error, this.providerName)
		}
	...
	async generateImage(
		...
		apiKey: string,
		inputImage?: string,
		taskId?: string, // novelweave_change
		...
		try {
			const response = await fetch(
				`${this.options.openRouterBaseUrl || "https://openrouter.ai/api/v1/"}chat/completions`, // novelweave_change: support baseUrl
				{
					method: "POST",
					headers: {
						// novelweave_change start
