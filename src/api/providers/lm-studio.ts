...
import { fetchWithTimeout, HeadersTimeoutError } from "./novelweave/fetchWithTimeout"

import { getModels, getModelsFromCache } from "./fetchers/modelCache"
import { handleOpenAIError } from "./utils/openai-error-handler"
import { getApiRequestTimeout } from "./utils/timeout-config" // novelweave_change
...
export class LmStudioHandler extends BaseProvider implements SingleCompletionHandler {
	...
	constructor(options: ApiHandlerOptions) {
		...
		const timeout = getApiRequestTimeout() // novelweave_change
	...
	override async *createMessage(
		...
		try {
			...
			// novelweave_change end
			throw new Error(
				"Please check the LM Studio developer logs to debug what went wrong. You may need to load the model with a larger context length to work with NovelWeave's prompts.",
			)
		}
	}
