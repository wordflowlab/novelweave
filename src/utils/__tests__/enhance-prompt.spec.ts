...
describe("enhancePrompt", () => {
	...
	// novelweave_change start - updated tests to work with createMessage fallback
	...
	it("handles streaming errors gracefully in fallback mode", async () => {
		...
		;(buildApiHandler as any).mockReturnValue({
			...
			createMessage: vi.fn().mockReturnValue(mockStream), // novelweave_change
			getModel: vi.fn().mockReturnValue({
				id: "test-model",
				info: {
					maxTokens: 4096,
	...
	// novelweave_change end - updated tests to work with createMessage fallback
