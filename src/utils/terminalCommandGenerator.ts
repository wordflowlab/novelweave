...
export async function generateTerminalCommand(options: TerminalCommandGeneratorOptions): Promise<void> {
	...
	try {
		...
		if (!activeTerminal) {
			vscode.window.showErrorMessage(t("novelweave:terminalCommandGenerator.noActiveTerminal"))
			return
		}
...
async function showWarningIfNeeded(context: vscode.ExtensionContext): Promise<boolean> {
	...
	if (!warningAcknowledged) {
		const warningChoice = await vscode.window.showInformationMessage(
			t("novelweave:terminalCommandGenerator.warningDialog.title"),
			{ modal: true, detail: t("novelweave:terminalCommandGenerator.warningDialog.message") },
			t("novelweave:terminalCommandGenerator.warningDialog.okButton"),
			t("novelweave:terminalCommandGenerator.warningDialog.cancelButton"),
		)

		if (warningChoice !== t("novelweave:terminalCommandGenerator.warningDialog.okButton")) {
			return false
		}
...
async function getUserInput(): Promise<string | undefined> {
	return await vscode.window.showInputBox({
		prompt: t("novelweave:terminalCommandGenerator.inputPrompt"),
		placeHolder: t("novelweave:terminalCommandGenerator.inputPlaceholder"),
		ignoreFocusOut: true,
	})
}
	...
	await vscode.window.withProgress(
		{
			...
			title: t("novelweave:terminalCommandGenerator.generatingProgress"),
			cancellable: false,
		},
		async () => {
			...
			vscode.window.showInformationMessage(
				t("novelweave:terminalCommandGenerator.commandGenerated", { command: cleanCommand }),
			)
		},
	)
}
...
function handleError(error: unknown, outputChannel: vscode.OutputChannel, context: string): void {
	...
	if (context === "generateTerminalCommand") {
		vscode.window.showErrorMessage(t("novelweave:terminalCommandGenerator.generationFailed", { error: errorMessage }))
	} else {
