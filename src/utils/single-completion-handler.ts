...
import { buildApiHandler, SingleCompletionHandler, ApiHandler } from "../api" //novelweave_change
...
export async function singleCompletionHandler(apiConfiguration: ProviderSettings, promptText: string): Promise<string> {
	...
	if (!("completePrompt" in handler)) {
		// novelweave_change start - stream responses for handlers without completePrompt
...
// novelweave_change start - Stream responses using createMessage
...
// novelweave_change end - streamResponseFromHandler
