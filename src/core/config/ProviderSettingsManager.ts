...
import { migrateMorphApiKey } from "./novelweave/migrateMorphApiKey"
...
export const providerProfilesSchema = z.object({
	...
	migrations: z
		.object({
			...
			morphApiKeyMigrated: z.boolean().optional(), // novelweave_change: Morph API key migration
		})
		.optional(),
})
			...
			return await this.lock(async () => {
				...
				if (!providerProfiles.migrations) {
					providerProfiles.migrations = {
						...
						morphApiKeyMigrated: false, // novelweave_change: Morph API key migration
