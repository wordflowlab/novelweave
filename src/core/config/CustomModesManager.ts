...
import axios from "axios" // novelweave_change

import { type ModeConfig, type PromptComponent, customModesSettingsSchema, modeConfigSchema } from "@roo-code/types"

import { fileExistsAtPath } from "../../utils/fs"
import { getWorkspacePath } from "../../utils/path"
import { getGlobalRooDirectory, getProjectRooDirectoryForCwd /*novelweave_change*/ } from "../../services/roo-config"
...
const ROOMODES_FILENAME = ".novelweavemodes"
...
export class CustomModesManager {
	...
	// novelweave_change start: Added organizationModes parameter and precedence logic
	...
	private async watchCustomModesFiles(): Promise<void> {
		...
		const handleSettingsChange = async () => {
			try {
				...
				// Get modes from .novelweavemodes if it exists (takes precedence)
				const roomodesPath = await this.getWorkspaceRoomodes()
				const roomodesModes = roomodesPath ? await this.loadModesFromFile(roomodesPath) : []

				// novelweave_change start Get organization modes from global state to preserve them
		...
		if (workspaceFolders && workspaceFolders.length > 0) {
			...
			const handleRoomodesChange = async () => {
				try {
					...
					console.error(`[CustomModesManager] Error handling .novelweavemodes file change:`, error)
				}
			}
	...
	public async getCustomModes(): Promise<ModeConfig[]> {
		...
		// Get modes from .novelweavemodes if it exists
		const roomodesPath = await this.getWorkspaceRoomodes()
		const roomodesModes = roomodesPath ? await this.loadModesFromFile(roomodesPath) : []

		// novelweave_change start: Get organization modes from global state
		...
		try {
			...
			if (isGlobalMode) {
				...
				modeRulesDir = path.join(
					getProjectRooDirectoryForCwd(workspacePath) /* novelweave_change */,
					`rules-${slug}`,
				)
			}
	...
	public async exportModeWithRules(slug: string, customPrompts?: PromptComponent): Promise<ExportResult> {
		try {
			...
			const modeRulesDir = isGlobalMode
				...
				: path.join(getProjectRooDirectoryForCwd(baseDir) /* novelweave_change */, `rules-${slug}`)
	...
	private async importRulesFiles(
		...
		if (source === "global") {
			...
			const workspacePath = getWorkspacePath()
			baseDir = getProjectRooDirectoryForCwd(workspacePath) // novelweave_change
			rulesFolderPath = path.join(baseDir, `rules-${importMode.slug}`)
		}
	...
	// novelweave_change start: New method to fetch organization modes
	/**
	 * Fetches custom modes for a specific organization from the NovelWeave API
	 * @param novelweaveToken - The authentication token
	 * @param organizationId - The organization ID (undefined for personal account)
	 * @param novelweaveTesterWarningsDisabledUntil - Timestamp for suppressing tester warnings
	 * @returns Array of organization-specific modes
	 */
	public async fetchOrganizationModes(
		novelweaveToken: string,
		organizationId?: string,
		novelweaveTesterWarningsDisabledUntil?: number,
	): Promise<ModeConfig[]> {
		try {
			...
			headers[X_NOVELWEAVE_ORGANIZATIONID] = organizationId

			// Add X-NOVELWEAVE-TESTER: SUPPRESS header if the setting is enabled
			if (novelweaveTesterWarningsDisabledUntil && novelweaveTesterWarningsDisabledUntil > Date.now()) {
	...
	// novelweave_change start: New method to refresh modes with organization modes
	/**
