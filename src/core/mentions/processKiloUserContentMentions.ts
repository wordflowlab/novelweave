...
export async function processKiloUserContentMentions({
	...
	const processUserContentMentions = async () => {
		...
		const { localWorkflowToggles, globalWorkflowToggles } = await refreshWorkflowToggles(context, cwd) // novelweave_change
			...
			userContent.map(async (block) => {
				...
				if (block.type === "text") {
					if (shouldProcessMentions(block.text)) {
						// novelweave_change begin: pull slash commands from Cline
