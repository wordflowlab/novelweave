...
describe("RooIgnoreController", () => {
	...
	describe("initialization", () => {
		/**
		 * Tests the controller initialization when .novelweaveignore exists
		 */
		it("should load .novelweaveignore patterns on initialization when file exists", async () => {
			// Setup mocks to simulate existing .novelweaveignore file
			...
			expect(mockFileExists).toHaveBeenCalledWith(path.join(TEST_CWD, ".novelweaveignore"))
			expect(mockReadFile).toHaveBeenCalledWith(path.join(TEST_CWD, ".novelweaveignore"), "utf8")
		...
		/**
		 * Tests the controller behavior when .novelweaveignore doesn't exist
		 */
		it("should allow all access when .novelweaveignore doesn't exist", async () => {
			// Setup mocks to simulate missing .novelweaveignore file
		 ...
		 */
		it("should set up file watcher for .novelweaveignore changes", async () => {
			// Check that watcher was created with correct pattern
			expect(vscode.workspace.createFileSystemWatcher).toHaveBeenCalledWith(
				expect.objectContaining({
					base: TEST_CWD,
					pattern: ".novelweaveignore",
				}),
			)
		 ...
		 */
		it("should handle errors when loading .novelweaveignore", async () => {
			...
			expect(consoleSpy).toHaveBeenCalledWith("Unexpected error loading .novelweaveignore:", expect.any(Error))
	...
	describe("validateAccess", () => {
		beforeEach(async () => {
			// Setup .novelweaveignore content
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules\n.git\nsecrets/**\n*.log")
			await controller.initialize()
		})
		...
		/**
		 * Tests the default behavior when no .novelweaveignore exists
		 */
		it("should allow all access when no .novelweaveignore content", async () => {
			// Create a new controller with no .novelweaveignore
			mockFileExists.mockResolvedValue(false)
	...
	describe("validateCommand", () => {
		...
		/**
		 * Tests behavior when no .novelweaveignore exists
		 */
		it("should allow all commands when no .novelweaveignore exists", async () => {
			// Create a new controller with no .novelweaveignore
			mockFileExists.mockResolvedValue(false)
			const emptyController = new RooIgnoreController(TEST_CWD)
			await emptyController.initialize()
	...
	describe("getInstructions", () => {
		/**
		 * Tests instructions generation with .novelweaveignore
		 */
		it("should generate formatted instructions when .novelweaveignore exists", async () => {
			// Setup .novelweaveignore content
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules\n.git\nsecrets/**")
			await controller.initialize()
			...
			expect(instructions).toContain("# .novelweaveignore")
		 ...
		 * Tests behavior when no .novelweaveignore exists
		 */
		it("should return undefined when no .novelweaveignore exists", async () => {
			// Setup no .novelweaveignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
	...
	describe("file watcher", () => {
		/**
		 * Tests behavior when .novelweaveignore is created
		 */
		it("should reload .novelweaveignore when file is created", async () => {
			// Setup initial state without .novelweaveignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
			...
			// Create and initialize controller with no .novelweaveignore
			controller = new RooIgnoreController(TEST_CWD)
			await controller.initialize()
			...
			// Force reload of .novelweaveignore content manually
			await controller.initialize()
		...
		/**
		 * Tests behavior when .novelweaveignore is changed
		 */
		it("should reload .novelweaveignore when file is changed", async () => {
			// Setup initial state with .novelweaveignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
		...
		/**
		 * Tests behavior when .novelweaveignore is deleted
		 */
		it("should reset when .novelweaveignore is deleted", async () => {
			// Setup initial state with .novelweaveignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
