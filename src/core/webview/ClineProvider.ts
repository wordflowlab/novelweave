...
import { McpDownloadResponse, McpMarketplaceCatalog } from "../../shared/novelweave/mcp"
import { McpServer } from "../../shared/mcp"
import { OpenRouterHandler } from "../../api/providers"
import { stringifyError } from "../../shared/novelweave/errorUtils"
import isWsl from "is-wsl"
import { getNovelweaveDefaultModel } from "../../api/providers/novelweave/getNovelweaveDefaultModel"
import { getNovelWeaveWrapperProperties } from "../../core/novelweave/wrapper"
	...
	implements vscode.WebviewViewProvider, TelemetryPropertiesProvider, TaskProviderLike
{
	...
	public skillsManager?: SkillsManager // novelweave_change: Skills support
	...
	*/
	...
	async resolveWebviewView(webviewView: vscode.WebviewView | vscode.WebviewPanel) {
		...
		// novelweave_change start: extract constant inTabMode
		...
		this.getState().then(
			({
				...
				terminalShellIntegrationDisabled = true, // novelweave_change: default
	...
	public async createTaskWithHistoryItem(historyItem: HistoryItem & { rootTask?: Task; parentTask?: Task }) {
		...
		const task = new Task({
			context: this.context, // novelweave_change
			provider: this,
			apiConfiguration,
			enableDiff,
			enableCheckpoints,
	...
	private async getHMRHtmlContent(webview: vscode.Webview): Promise<string> {
		...
		const iconsUri = getUri(webview, this.contextProxy.extensionUri, ["assets", "icons"]) // novelweave_change
		...
		const csp = [
			...
			`img-src ${webview.cspSource} https://storage.googleapis.com https://img.clerk.com data: https://*.googleusercontent.com https://*.googleapis.com https://*.githubusercontent.com`, // novelweave_change: add https://*.googleusercontent.com and https://*.googleapis.com and https://*.githubusercontent.com
			`media-src ${webview.cspSource}`,
			`script-src 'unsafe-eval' ${webview.cspSource} https://* https://*.posthog.com http://${localServerUrl} http://0.0.0.0:${localPort} 'nonce-${nonce}'`,
			`connect-src ${webview.cspSource} https://* http://localhost:3000 https://*.posthog.com ws://${localServerUrl} ws://0.0.0.0:${localPort} http://${localServerUrl} http://0.0.0.0:${localPort}`, // novelweave_change: add http://localhost:3000
		]
			...
			<html lang="en">
				<head>
					...
					<script nonce="${nonce}">
						...
						window.ICONS_BASE_URI = "${iconsUri}" // novelweave_change
						window.AUDIO_BASE_URI = "${audioUri}"
						window.MATERIAL_ICONS_BASE_URI = "${materialIconsUri}"
					</script>
					<title>NovelWeave</title>
				</head>
				<body>
					<div id="root"></div>
					${reactRefresh}
	...
	private getHtmlContent(webview: vscode.Webview): string {
		...
		const imagesUri = getUri(webview, this.contextProxy.extensionUri, ["assets", "images"])
		const iconsUri = getUri(webview, this.contextProxy.extensionUri, ["assets", "icons"]) // novelweave_changes
		const audioUri = getUri(webview, this.contextProxy.extensionUri, ["webview-ui", "audio"])
		...
		*/
        ...
        <html lang="en">
          <head>
			...
			<!-- novelweave_change: add https://*.googleusercontent.com https://*.googleapis.com https://*.githubusercontent.com to img-src, https://*, http://localhost:3000 to connect-src -->
	...
	async upsertProviderProfile(
		...
		try {
			...
			if (activate) {
				...
				await TelemetryService.instance.updateIdentity(providerSettings.novelweaveToken ?? "") // novelweave_change
			} else {
				await this.updateGlobalState("listApiConfigMeta", await this.providerSettingsManager.listConfig())
			}
	...
	async ensureMcpServersDirectoryExists(): Promise<string> {
		...
		if (process.platform === "win32") {
			// Windows: %APPDATA%\NovelWeave-Code\MCP
			mcpServersDir = path.join(os.homedir(), "AppData", "Roaming", "NovelWeave-Code", "MCP")
		} else if (process.platform === "darwin") {
			// macOS: ~/Documents/NovelWeave-Code/MCP
			mcpServersDir = path.join(os.homedir(), "Documents", "NovelWeave-Code", "MCP")
		} else {
			// Linux: ~/.local/share/NovelWeave-Code/MCP
			mcpServersDir = path.join(os.homedir(), ".local", "share", "NovelWeave-Code", "MCP")
		}
		...
		try {
			...
			return path.join(os.homedir(), ".novelweave", "mcp")
		}
	...
	// novelweave_change:
	async handleNovelWeaveCallback(token: string) {
		const novelweave: ProviderName = "novelweave"
		let { apiConfiguration, currentApiConfigName = "default" } = await this.getState()

		await this.upsertProviderProfile(currentApiConfigName, {
			...apiConfiguration,
			apiProvider: "novelweave",
			novelweaveToken: token,
		})

		vscode.window.showInformationMessage("NovelWeave successfully configured!")
	...
	async getTaskWithId(id: string): Promise<{
		...
		if (historyItem) {
			...
			if (fileExists) {
				...
				) //novelweave_change show extra debugging information to debug task not found issues
			}
		} else {
			vscode.window.showErrorMessage(`Task with ID: ${id} not found in history.`) // novelweave_change show extra debugging information to debug task not found issues
		}

		// if we tried to get a task that doesn't exist, remove it from state
		// FIXME: this seems to happen sometimes when the json file doesnt save to disk for some reason
		// await this.deleteTaskFromState(id) // novelweave_change disable confusing behaviour
		await this.setTaskFileNotFound(id) // novelweave_change
	...
	async getStateToPostToWebview(): Promise<ExtensionState> {
		const {
			...
			terminalZdotdir,
			fuzzyMatchThreshold,
			// mcpEnabled,  // novelweave_change: always true
			...
			ghostServiceSettings, // novelweave_changes
			...
			dismissedNotificationIds, // novelweave_change
			morphApiKey, // novelweave_change
		...
		const telemetryKey = process.env.NOVELWEAVE_POSTHOG_API_KEY
		...
		// novelweave_change start wrapper information
		const kiloCodeWrapperProperties = getNovelWeaveWrapperProperties()
		...
		return {
			...
			uiKind: vscode.UIKind[vscode.env.uiKind], // novelweave_change
			kiloCodeWrapperProperties, // novelweave_change wrapper information
			novelweaveDefaultModel: await getNovelweaveDefaultModel(
			...
			taskHistoryFullLength: taskHistory.length, // novelweave_change
			taskHistoryVersion: this.kiloCodeTaskHistoryVersion, // novelweave_change
			soundEnabled: soundEnabled ?? false,
			ttsEnabled: ttsEnabled ?? false,
			ttsSpeed: ttsSpeed ?? 1.0,
			diffEnabled: diffEnabled ?? true,
			enableCheckpoints: enableCheckpoints ?? true,
			shouldShowAnnouncement: false, // novelweave_change
			...
			terminalShellIntegrationDisabled: terminalShellIntegrationDisabled ?? true, // novelweave_change: default
			...
			mcpEnabled: true, // novelweave_change: always true
			...
			showAutoApproveMenu: showAutoApproveMenu ?? false, // novelweave_change
			showTaskTimeline: showTaskTimeline ?? true, // novelweave_change
			showTimestamps: showTimestamps ?? true, // novelweave_change
			hideCostBelowThreshold, // novelweave_change
			language, // novelweave_change
			renderContext: this.renderContext,
			maxReadFileLine: maxReadFileLine ?? -1,
			maxImageFileSize: maxImageFileSize ?? 5,
			maxTotalImageSize: maxTotalImageSize ?? 20,
			maxConcurrentFileReads: maxConcurrentFileReads ?? 5,
			allowVeryLargeReads: allowVeryLargeReads ?? false, // novelweave_change
			...
			systemNotificationsEnabled: systemNotificationsEnabled ?? false, // novelweave_change
			dismissedNotificationIds: dismissedNotificationIds ?? [], // novelweave_change
			morphApiKey, // novelweave_change
			fastApplyModel: fastApplyModel ?? "auto", // novelweave_change: Fast Apply model selection
	...
	> {
		...
		const apiProvider: ProviderName = stateValues.apiProvider ? stateValues.apiProvider : "novelweave" // novelweave_change: fall back to novelweave
		...
		return {
			...
			novelweaveDefaultModel: await getNovelweaveDefaultModel(
				providerSettings.novelweaveToken,
				providerSettings.novelweaveOrganizationId,
			), // novelweave_change
			...
			alwaysAllowUpdateTodoList: stateValues.alwaysAllowUpdateTodoList ?? true, // novelweave_change
			...
			// taskHistory: stateValues.taskHistory ?? [], // novelweave_change
			...
			terminalShellIntegrationDisabled: stateValues.terminalShellIntegrationDisabled ?? true, // novelweave_change: default
			...
			commitMessageApiConfigId: stateValues.commitMessageApiConfigId, // novelweave_change
			terminalCommandApiConfigId: stateValues.terminalCommandApiConfigId, // novelweave_change
			// novelweave_change start
			...
			showAutoApproveMenu: stateValues.showAutoApproveMenu ?? false, // novelweave_change
			showTaskTimeline: stateValues.showTaskTimeline ?? true, // novelweave_change
			showTimestamps: stateValues.showTimestamps ?? true, // novelweave_change
			hideCostBelowThreshold: stateValues.hideCostBelowThreshold ?? 0, // novelweave_change
			maxReadFileLine: stateValues.maxReadFileLine ?? -1,
			maxImageFileSize: stateValues.maxImageFileSize ?? 5,
			maxTotalImageSize: stateValues.maxTotalImageSize ?? 20,
			maxConcurrentFileReads: stateValues.maxConcurrentFileReads ?? 5,
			allowVeryLargeReads: stateValues.allowVeryLargeReads ?? false, // novelweave_change
			systemNotificationsEnabled: stateValues.systemNotificationsEnabled ?? true, // novelweave_change
			dismissedNotificationIds: stateValues.dismissedNotificationIds ?? [], // novelweave_change
			morphApiKey: stateValues.morphApiKey, // novelweave_change
			fastApplyModel: stateValues.fastApplyModel ?? "auto", // novelweave_change: Fast Apply model selection
	...
	async resetState() {
		...
		// Logout from NovelWeave provider before resetting (same approach as ProfileView logout)
		const { apiConfiguration, currentApiConfigName = "default" } = await this.getState()
		if (apiConfiguration.novelweaveToken) {
			await this.upsertProviderProfile(currentApiConfigName, {
				...apiConfiguration,
				novelweaveToken: "",
			})
	...
	private getAppProperties(): StaticAppProperties {
		if (!this._appProperties) {
			...
			} = getNovelWeaveWrapperProperties()
			// novelweave_change end
			...
			this._appProperties = {
				...
				platform: isWsl ? "wsl" /* novelweave_change */ : process.platform,
				editorName: kiloCodeWrapperTitle ? kiloCodeWrapperTitle : vscode.env.appName, // novelweave_change
				wrapped: kiloCodeWrapped, // novelweave_change
				wrapper: kiloCodeWrapper, // novelweave_change
				wrapperCode: kiloCodeWrapperCode, // novelweave_change
				wrapperVersion: kiloCodeWrapperVersion, // novelweave_change
				wrapperTitle: kiloCodeWrapperTitle, // novelweave_change
			}
		}
	...
	public async getTelemetryProperties(): Promise<TelemetryProperties> {
		...
		function getOpenRouter() {
			if (
				apiConfiguration &&
				(apiConfiguration.apiProvider === "openrouter" || apiConfiguration.apiProvider === "novelweave")
			) {
				return {
					openRouter: {
		...
		return {
			...
			// ...this.getCloudProperties(), novelweave_change: disable
			// novelweave_change start
			...(await getModelId()),
			...getMemory(),
			...getFastApply(),
