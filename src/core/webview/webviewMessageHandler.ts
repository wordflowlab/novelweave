...
import { showSystemNotification } from "../../integrations/notifications" // novelweave_change
import { singleCompletionHandler } from "../../utils/single-completion-handler" // novelweave_change
...
import { seeNewChanges } from "../checkpoints/novelweave/seeNewChanges" // novelweave_change
import { getTaskHistory } from "../../shared/novelweave/getTaskHistory" // novelweave_change
...
export const webviewMessageHandler = async (
	...
	switch (message.type) {
		case "webviewDidLaunch":
			...
			// novelweave_change start: Fetch organization modes on startup
			...
			const { refreshWorkflowToggles } = await import("../context/instructions/workflows") // novelweave_change
			await refreshWorkflowToggles(provider.context, provider.cwd) // novelweave_change

			provider.postStateToWebview()
			provider.postRulesDataToWebview() // novelweave_change: send workflows and rules immediately
			...
			provider.getStateToPostToWebview().then(async (/*novelweave_change*/ state) => {
				const { telemetrySetting } = state
				const isOptedIn = telemetrySetting !== "disabled"
				TelemetryService.instance.updateTelemetryState(isOptedIn)
				await TelemetryService.instance.updateIdentity(state.apiConfiguration.novelweaveToken ?? "") // novelweave_change
			})
		...
		case "requestRouterModels":
			...
			const routerModels: Record<RouterName, ModelRecord> = {
				openrouter: {},
				gemini: {}, // novelweave_change
				"vercel-ai-gateway": {},
				huggingface: {},
				litellm: {},
				"novelweave-openrouter": {}, // novelweave_change
				deepinfra: {},
				"io-intelligence": {},
				requesty: {},
				unbound: {},
				glama: {},
				chutes: {}, // novelweave_change
				ollama: {},
				lmstudio: {},
				ovhcloud: {}, // novelweave_change
			}
			...
			// novelweave_change start: openrouter auth, novelweave provider
			...
			const modelFetchPromises: Array<{ key: RouterName; options: GetModelsOptions }> = [
				...
				{ key: "chutes", options: { provider: "chutes", apiKey: apiConfiguration.chutesApiKey } }, // novelweave_change
				{
					key: "novelweave-openrouter",
					options: {
						provider: "novelweave-openrouter",
						novelweaveToken: apiConfiguration.novelweaveToken,
						novelweaveOrganizationId: apiConfiguration.novelweaveOrganizationId,
					},
				},
		...
		case "requestOllamaModels": {
			...
			try {
				...
				const ollamaModels = await getModels({
					...
					numCtx: ollamaApiConfig.ollamaNumCtx, // novelweave_change
				})
		...
		case "openProjectMcpSettings": {
			...
			const rooDir = path.join(workspaceFolder, ".novelweave")
		...
		case "openGlobalKeybindings":
			vscode.commands.executeCommand("workbench.action.openGlobalKeybindings", message.text ?? "novelweave.")
			break
		...
		case "enhancementApiConfigId":
			...
			await provider.postStateToWebview()
			break
		// novelweave_change start - commitMessageApiConfigId
		case "commitMessageApiConfigId":
			await updateGlobalState("commitMessageApiConfigId", message.text)
			await provider.postStateToWebview()
			break
		// novelweave_change end - commitMessageApiConfigId
		// novelweave_change start - terminalCommandApiConfigId
		case "terminalCommandApiConfigId":
			await updateGlobalState("terminalCommandApiConfigId", message.text)
			await provider.postStateToWebview()
			break
		// novelweave_change end - terminalCommandApiConfigId
		// novelweave_change start - ghostServiceSettings
		case "ghostServiceSettings":
			...
			await provider.postStateToWebview()
			vscode.commands.executeCommand("novelweave.ghost.reload")
			break
		// novelweave_change end
		...
		case "enhancePrompt":
			if (message.text) {
				try {
					...
					TelemetryService.instance.captureException(error, { context: "enhance_prompt" }) // novelweave_change
		...
		case "showFeedbackOptions": {
			...
			if (answer === githubIssuesText) {
				...
				await vscode.env.openExternal(vscode.Uri.parse("https://novelweave.ai/support"))
			}
			break
		}
		// novelweave_change end
		...
		case "upsertApiConfiguration":
			// novelweave_change start: check for novelweaveToken change to remove organizationId and fetch organization modes
			if (message.text && message.apiConfiguration) {
				...
				try {
					...
					organizationChanged =
						currentConfig.novelweaveOrganizationId !== message.apiConfiguration.novelweaveOrganizationId
					...
					if (organizationChanged) {
						...
						await flushModels("novelweave-openrouter")
						const models = await getModels({
							provider: "novelweave-openrouter",
							novelweaveOrganizationId: message.apiConfiguration.novelweaveOrganizationId,
							novelweaveToken: message.apiConfiguration.novelweaveToken,
						})
						provider.postMessageToWebview({
							type: "routerModels",
							routerModels: { "novelweave-openrouter": models } as Record<RouterName, ModelRecord>,
						})
					}
				} catch (error) {
			...
			// novelweave_change end: check for novelweaveToken change to remove organizationId and fetch organization modes
			break
		...
		// novelweave_change_start
		case "fetchProfileDataRequest":
			try {
				const { apiConfiguration, currentApiConfigName } = await provider.getState()
				const novelweaveToken = apiConfiguration?.novelweaveToken

				if (!novelweaveToken) {
					provider.log("NovelWeave token not found in extension state.")
					provider.postMessageToWebview({
						type: "profileDataResponse",
						payload: { success: false, error: "NovelWeave API token not configured." },
					})
					break
				}
				...
				if (
					apiConfiguration.novelweaveTesterWarningsDisabledUntil &&
				...
				const organizationExists = (response.data.organizations ?? []).some(
					({ id }) => id === apiConfiguration?.novelweaveOrganizationId,
				)
				if (apiConfiguration?.novelweaveOrganizationId && !organizationExists) {
				...
				provider.postMessageToWebview({
					...
					payload: { success: true, data: { novelweaveToken, ...response.data } },
				})
			} catch (error: any) {
				const errorMessage =
		...
		case "fetchBalanceDataRequest": // New handler
			try {
				const { apiConfiguration } = await provider.getState()
				const { novelweaveToken, novelweaveOrganizationId } = apiConfiguration ?? {}

				if (!novelweaveToken) {
					provider.log("NovelWeave token not found in extension state for balance data.")
					provider.postMessageToWebview({
				...
				if (novelweaveOrganizationId) {
					headers["X-NovelWeave-OrganizationId"] = novelweaveOrganizationId
				}
		...
		case "shopBuyCredits": // New handler
			try {
				...
				const novelweaveToken = apiConfiguration?.novelweaveToken
				if (!novelweaveToken) {
					provider.log("NovelWeave token not found in extension state for buy credits.")
					break
				}
		...
		case "createRuleFile": {
			if (
				...
				try {
					...
					vscode.window.showErrorMessage(t("novelweave:rules.errors.failedToCreateRuleFile"))
				}
				await provider.postRulesDataToWebview()
			}
			break
		...
		case "deleteRuleFile": {
			if (message.rulePath) {
				try {
					...
					vscode.window.showErrorMessage(t("novelweave:rules.errors.failedToDeleteRuleFile"))
				}
				await provider.postRulesDataToWebview()
			}
			break
		...
		// novelweave_change start - add clearUsageData
		case "clearUsageData": {
			try {
		...
		// novelweave_change start - add getUsageData
		case "getUsageData": {
			if (message.text) {
				try {
					const usageTracker = UsageTracker.getInstance()
		...
		// novelweave_change end - add getUsageData
		// novelweave_change start - add toggleTaskFavorite
		case "toggleTaskFavorite":
			if (message.text) {
				await provider.toggleTaskFavorite(message.text)
			}
			break
		// novelweave_change start - add fixMermaidSyntax
		...
		case "fetchNovelweaveNotifications": {
			await fetchNovelweaveNotificationsHandler(provider)
			break
		}
