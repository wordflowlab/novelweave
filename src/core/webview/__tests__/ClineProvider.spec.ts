...
} from "@roo-code/types" // novelweave_change: openRouterDefaultModelId
...
vi.mock("vscode", () => ({
	...
	window: {
		...
		createTextEditorDecorationType: vi.fn(() => ({ dispose: vi.fn() })), // novelweave_change
	},
	workspace: {
	...
	env: {
		uriScheme: "vscode",
		language: "en",
		uiKind: 1, // novelweave_change Desktop
...
describe("ClineProvider", () => {
	...
	test("resolveWebviewView sets up webview correctly in development mode even if local server is not running", async () => {
		...
		expect(mockWebviewView.webview.html).toContain(
			// novelweave_change: added localhost:3000
	...
	test("postMessageToWebview sends message to webview", async () => {
		...
		const mockState: ExtensionState = {
			...
			apiConfiguration: {
				...
				novelweaveModel: openRouterDefaultModelId,
				novelweaveToken: "novelweave-token",
				// novelweave_change end
			},
	...
	test("getState returns correct initial state", async () => {
		...
		// expect(state).toHaveProperty("taskHistory") // novelweave_change
...
describe("Project MCP Settings", () => {
	...
	test.skip("handles openProjectMcpSettings message", async () => {
		...
		expect(mockedFs.mkdir).toHaveBeenCalledWith("/test/workspace/.novelweave", { recursive: true })

		// Verify file was created with default content
		expect(safeWriteJson).toHaveBeenCalledWith("/test/workspace/.roo/mcp.json", { mcpServers: {} })

		// Check that openFile was called
		expect(openFileSpy).toHaveBeenCalledWith("/test/workspace/.novelweave/mcp.json")
	})
	...
	test.skip("handles openProjectMcpSettings file creation error", async () => {
		...
		expect(vscode.window.showErrorMessage).toHaveBeenCalledWith(
			// novelweave_change
			expect.stringContaining("Failed to create or open .novelweave/mcp.json"),
		)
	})
})
...
describe("ClineProvider - Router Models", () => {
	...
	test("handles requestRouterModels with successful responses", async () => {
		...
		expect(getModels).toHaveBeenCalledWith({ provider: "openrouter", apiKey: "openrouter-key" }) // novelweave_change: apiKey
		// novelweave_change start
		...
		expect(getModels).toHaveBeenCalledWith({ provider: "chutes", apiKey: "chutes-key" }) // novelweave_change
		expect(getModels).toHaveBeenCalledWith({ provider: "vercel-ai-gateway" })
		expect(getModels).toHaveBeenCalledWith({ provider: "ovhcloud", apiKey: "ovhcloud-key" }) // novelweave_change
		expect(getModels).toHaveBeenCalledWith({
			provider: "litellm",
			apiKey: "litellm-key",
		...
		expect(mockPostMessage).toHaveBeenCalledWith({
			...
			routerModels: {
				...
				gemini: mockModels, // novelweave_change
				requesty: mockModels,
				glama: mockModels,
				unbound: mockModels,
				chutes: mockModels, // novelweave_change
				litellm: mockModels,
				"novelweave-openrouter": mockModels,
				ollama: mockModels, // novelweave_change
				lmstudio: {},
				"vercel-ai-gateway": mockModels,
				ovhcloud: mockModels, // novelweave_change
				huggingface: {},
				"io-intelligence": {},
			},
		})
	...
	test("handles requestRouterModels with individual provider failures", async () => {
		...
		vi.mocked(getModels)
			...
			.mockResolvedValueOnce(mockModels) // novelweave_change: gemini success
			.mockRejectedValueOnce(new Error("Requesty API error")) // requesty fail
			.mockResolvedValueOnce(mockModels) // glama success
			.mockRejectedValueOnce(new Error("Unbound API error")) // unbound fail
			.mockRejectedValueOnce(new Error("Chutes API error")) // novelweave_change: chutes fail
			.mockRejectedValueOnce(new Error("Novelweave-OpenRouter API error")) // novelweave-openrouter fail
			.mockRejectedValueOnce(new Error("Ollama API error")) // novelweave_change
			.mockResolvedValueOnce(mockModels) // vercel-ai-gateway success
			.mockResolvedValueOnce(mockModels) // novelweave_change: ovhcloud
		...
		expect(mockPostMessage).toHaveBeenCalledWith({
			type: "singleRouterModelFetchResponse",
			success: false,
			error: "Novelweave-OpenRouter API error",
			values: { provider: "novelweave-openrouter" },
		})
