...
describe("webviewMessageHandler - requestRouterModels", () => {
	...
	it("successfully fetches models from all providers", async () => {
		...
		expect(mockGetModels).toHaveBeenCalledWith({ provider: "openrouter", apiKey: "openrouter-key" }) // novelweave_change: apiKey
	...
	it("handles LiteLLM models with values from message when config is missing", async () => {
		mockClineProvider.getState = vi.fn().mockResolvedValue({
			apiConfiguration: {
				...
				glamaApiKey: "glama-key",
				unboundApiKey: "unbound-key",
				ovhCloudAiEndpointsApiKey: "ovhcloud-key", // novelweave_change
	...
	it("handles individual provider failures gracefully", async () => {
		...
		mockGetModels
			...
			.mockResolvedValueOnce(mockModels) // novelweave_change: gemini
			.mockRejectedValueOnce(new Error("Requesty API error")) // requesty
			.mockResolvedValueOnce(mockModels) // glama
			.mockRejectedValueOnce(new Error("Unbound API error")) // unbound
			.mockRejectedValueOnce(new Error("Chutes API error")) // chutes // novelweave_change
			.mockResolvedValueOnce(mockModels) // novelweave-openrouter
	...
	it("handles Error objects and string errors correctly", async () => {
		...
		mockGetModels
			...
			.mockRejectedValueOnce(new Error("Gemini API error")) // // novelweave_change: gemini
			.mockRejectedValueOnce(new Error("Requesty API error")) // requesty
			.mockRejectedValueOnce(new Error("Glama API error")) // glama
			.mockRejectedValueOnce(new Error("Unbound API error")) // unbound
			.mockRejectedValueOnce(new Error("Chutes API error")) // chutes // novelweave_change
			.mockResolvedValueOnce({}) // novelweave-openrouter - Success
			...
			.mockRejectedValueOnce(new Error("OVHcloud AI Endpoints error")) // ovhcloud // novelweave_change
			.mockRejectedValueOnce(new Error("LiteLLM connection failed")) // litellm
