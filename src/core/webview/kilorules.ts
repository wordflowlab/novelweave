...
import { allowedExtensions } from "../../shared/novelweave/rules"
...
export async function createRuleFile(
	...
	if (!workspacePath && !isGlobal) {
		vscode.window.showErrorMessage(t("novelweave:rules.errors.noWorkspaceFound"))
		return
	}
	...
	if (await fileExistsAtPath(filePath)) {
		vscode.window.showErrorMessage(t("novelweave:rules.errors.fileAlreadyExists", { filename }))
		return
	}
...
${t("novelweave:rules.templates.workflow.description")}

${t("novelweave:rules.templates.workflow.stepsHeader")}

1. ${t("novelweave:rules.templates.workflow.step1")}
2. ${t("novelweave:rules.templates.workflow.step2")}
`
}
...
${t("novelweave:rules.templates.rule.description")}

${t("novelweave:rules.templates.rule.guidelinesHeader")}

- ${t("novelweave:rules.templates.rule.guideline1")}
- ${t("novelweave:rules.templates.rule.guideline2")}
`
}

export async function deleteRuleFile(rulePath: string): Promise<void> {
	const deleteAction = t("novelweave:rules.actions.delete")
	const filename = path.basename(rulePath)
	const result = await vscode.window.showWarningMessage(
		t("novelweave:rules.actions.confirmDelete", { filename }),
		{ modal: true },
		deleteAction,
	)
	...
	if (result === deleteAction) {
		...
		vscode.window.showInformationMessage(t("novelweave:rules.actions.deleted", { filename }))
	}
}
