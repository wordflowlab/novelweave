...
import { isAnyRecognizedNovelWeaveError, isPaymentRequiredError } from "../../shared/novelweave/errorUtils"
...
export interface TaskOptions extends CreateTaskOptions {
	context: vscode.ExtensionContext // novelweave_change
...
type UserContent = Array<Anthropic.ContentBlockParam> // novelweave_change

export class Task extends EventEmitter<TaskEvents> implements TaskLike {
	private context: vscode.ExtensionContext // novelweave_change

	readonly taskId: string
	private taskIsFavorited?: boolean // novelweave_change
	...
	constructor({
		...
		super()
		this.context = context // novelweave_change
		...
		this.taskIsFavorited = historyItem?.isFavorited // novelweave_change
		...
		this.workspacePath = parentTask
			...
			: (workspacePath ?? getWorkspacePath(path.join(os.homedir(), "Documents"))) // novelweave_change: use Documents instead of Desktop as default
	...
	private async addToClineMessages(message: ClineMessage) {
		...
		// novelweave_change start: no cloud service
	...
	async ask(
		...
		if (this.abort) {
			throw new Error(`[NovelWeave#ask] task ${this.taskId}.${this.instanceId} aborted`)
		}
	...
	handleWebviewAskResponse(askResponse: ClineAskResponse, text?: string, images?: string[]) {
		// this.askResponse = askResponse novelweave_change
	...
	async sayAndCreateMissingParamError(toolName: ToolName, paramName: string, relPath?: string) {
		const novelweaveExtraText = (() => {
			switch (toolName) {
				case "apply_diff":
					return t("novelweave:task.disableApplyDiff") + " "
				case "edit_file":
					return t("novelweave:task.disableEditFile") + " "
				default:
					return ""
			}
		})()
	...
	public async recursivelyMakeClineRequests(
		...
		while (stack.length > 0) {
			...
			if (this.abort) {
				throw new Error(
					`[NovelWeave#recursivelyMakeClineRequests] task ${this.taskId}.${this.instanceId} aborted`,
				)
			}
			...
			if (needsRulesFileCheck) {
				await this.say(
					"error",
					"Issue with processing the /newrule command. Double check that, if '.novelweave/rules' already exists, it's a directory and not a file. Otherwise there was an issue referencing this file/directory",
				)
			}
			// novelweave_change end
			...
			try {
					...
					while (!item.done) {
						...
						switch (chunk.type) {
							...
							case "usage":
								...
								inferenceProvider = chunk.inferenceProvider // novelweave_change
								break
							case "grounding":
				...
				if (assistantMessage.length > 0) {
					...
					await this.say(
						"error",
						t("novelweave:task.noAssistantMessages"), // novelweave_change
					)
	...
	/*private novelweave_change*/ async getSystemPrompt(): Promise<string> {
		...
		return await (async () => {
			...
			return SYSTEM_PROMPT(
				provider.context,
				this.cwd,
				// novelweave_change: supports images => supports browser
	...
	public async *attemptApiRequest(retryAttempt: number = 0): ApiStream {
		...
		if (Task.lastGlobalApiRequestTime) {
			const now = performance.now() // novelweave_change
		...
		Task.lastGlobalApiRequestTime = performance.now() // novelweave_change
		...
		try {
			...
			this.isWaitingForFirstChunk = false
			// novelweave_change start
			if (apiConfiguration?.apiProvider === "novelweave" && isAnyRecognizedNovelWeaveError(error)) {
				const { response } = await (isPaymentRequiredError(error)
					? this.ask(
							"payment_required_prompt",
							JSON.stringify({
								title: error.error?.title ?? t("novelweave:lowCreditWarning.title"),
								message: error.error?.message ?? t("novelweave:lowCreditWarning.message"),
								balance: error.error?.balance ?? "0.00",
								buyCreditsUrl: error.error?.buyCreditsUrl ?? "https://novelweave.ai/profile",
							}),
						)
					: this.ask(
							"invalid_model",
							JSON.stringify({
								modelId: apiConfiguration.novelweaveModel,
								error: {
									status: error.status,
	...
	public recordToolError(toolName: ToolName, error?: string) {
		...
		TelemetryService.instance.captureEvent(TelemetryEventName.TOOL_ERROR, { toolName, error }) // novelweave_change
	}
