...
import { editFileTool } from "../tools/editFileTool" // novelweave_change: Morph fast apply
...
import { newRuleTool } from "../tools/newRuleTool" // novelweave_change
import { reportBugTool } from "../tools/reportBugTool" // novelweave_change
import { condenseTool } from "../tools/condenseTool" // novelweave_change
import { codebaseSearchTool } from "../tools/codebaseSearchTool"
import { experiments, EXPERIMENT_IDS } from "../../shared/experiments"
import { applyDiffToolLegacy } from "../tools/applyDiffTool"
import { yieldPromise } from "../novelweave"
...
export async function presentAssistantMessage(cline: Task, recursionDepth: number = 0 /*novelweave_change*/) {
	if (cline.abort) {
	...
	switch (block.type) {
		...
		case "tool_use":
			...
			await checkpointSaveAndMark(cline) // novelweave_change: moved out of switch
			switch (block.name) {
				case "write_to_file":
					// await checkpointSaveAndMark(cline) // novelweave_change
	...
	if (!block.partial || cline.didRejectTool || cline.didAlreadyUseTool) {
		...
		if (cline.currentStreamingContentIndex < cline.assistantMessageContent.length) {
			...
			// novelweave_change start: prevent excessive recursion
			await yieldPromise()
...
async function checkpointSaveAndMark(task: Task) {
	...
	try {
		// novelweave_change: order changed to prevent second execution while still awaiting the save
