...
export async function getCheckpointService(
	...
	try {
		...
		task.enableCheckpoints = false
		reportError("Task#getCheckpointService", err) // novelweave_change
...
async function checkGitInstallation(
	...
	try {
		...
		service.on("checkpoint", ({ fromHash: from, toHash: to, suppressMessage }) => {
			try {
				...
				task.say(
					...
					console.error(err)
					reportError("getCheckpointService:say('checkpoint_saved')", err) // novelweave_change
				})
			} catch (err) {
				log("[Task#getCheckpointService] caught unexpected error in on('checkpoint'), disabling checkpoints")
				console.error(err)
				task.enableCheckpoints = false
				reportError("getCheckpointService:on('checkpoint')", err) // novelweave_change
			}
		})
		...
		try {
			...
			task.enableCheckpoints = false
			reportError("getCheckpointService:initShadowGit", err) // novelweave_change
		}
	} catch (err) {
		...
		task.enableCheckpoints = false
		task.checkpointServiceInitializing = false
		reportError("getCheckpointService", err) // novelweave_change
	}
}
...
export async function checkpointSave(task: Task, force = false, suppressMessage = false) {
	...
	return service
		...
		.catch((err) => {
			...
			task.enableCheckpoints = false
			reportError("checkpointSave", err) // novelweave_change
		})
}
...
export async function checkpointRestore(
	...
	try {
		...
		task.enableCheckpoints = false
		reportError("checkpointRestore", err) // novelweave_change
	}
}
...
export async function checkpointDiff(task: Task, { ts, previousCommitHash, commitHash, mode }: CheckpointDiffOptions) {
	...
	try {
		...
		task.enableCheckpoints = false
		reportError("checkpointDiff", err) // novelweave_change
	}
}
