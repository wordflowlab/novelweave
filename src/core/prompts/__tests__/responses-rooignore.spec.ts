...
describe("RooIgnore Response Formatting", () => {
	...
	describe("formatResponse.rooIgnoreError", () => {
		...
		it("should format error message for ignored files", () => {
			...
			expect(errorMessage).toContain(
				"Access to secrets/api-keys.json is blocked by the .novelweaveignore file settings",
			)
			expect(errorMessage).toContain("continue in the task without using this file")
			expect(errorMessage).toContain("ask the user to update the .novelweaveignore file")
		})
	...
	describe("getInstructions", () => {
		 ...
		 */
		it("should format .novelweaveignore instructions for the LLM", async () => {
			// Create controller
			const controller = new RooIgnoreController(TEST_CWD)
			await controller.initialize()
		...
		it("should return undefined when no .novelweaveignore exists", async () => {
			// Set up no .novelweaveignore
			mockFileExists.mockResolvedValue(false)

			// Create controller without .novelweaveignore
			const controller = new RooIgnoreController(TEST_CWD)
			await controller.initialize()
