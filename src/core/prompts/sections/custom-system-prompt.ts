...
export function getSystemPromptFilePath(cwd: string, mode: Mode): string {
	// novelweave_change
	return path.join(cwd, ".novelweave", `system-prompt-${mode}`)
}

/**
 * Loads custom system prompt from a file at .novelweave/system-prompt-[mode slug]
...
/**
 * Ensures the .novelweave directory exists, creating it if necessary
 */
export async function ensureRooDirectory(cwd: string): Promise<void> {
	// novelweave_change
	const rooDir = path.join(cwd, ".novelweave")
