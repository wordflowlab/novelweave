...
let hasShownNonNovelweaveRulesMessage = false
...
import { ClineRulesToggles } from "../../../shared/cline-rules" // novelweave_change
...
async function readTextFilesFromDirectory(dirPath: string): Promise<Array<{ filename: string; content: string }>> {
	try {
		...
		// novelweave_change, must be imported at submodule level because the module is imported in the webview-ui
...
export async function loadRuleFiles(cwd: string): Promise<string> {
	...
	const ruleFiles = [".novelweaverules", ".roorules", ".clinerules"]

	for (const file of ruleFiles) {
		const content = await safeReadFile(path.join(cwd, file))
		if (content) {
			if (file !== ".novelweaverules" && vscodeAPI && !hasShownNonNovelweaveRulesMessage) {
				// novelweave_change: show message to move to .novelweave/rules/
				vscodeAPI.window.showWarningMessage(
					`Loading non-Novelweave rules from ${file}, consider moving to .novelweave/rules/`,
				)
				hasShownNonNovelweaveRulesMessage = true
			} // novelweave_change end
			return `\n# Rules from ${file}:\n${content}\n`
		}
	}
...
export async function addCustomInstructions(
	...
	cwd: string,
	mode: string,
	// novelweave_change begin: rule toggles
	options: {
		language?: string
	...
	if (mode) {
		...
		if (modeRules.length > 0) {
			...
			const rooModeRuleFile = `.novelweaverules-${mode}`
	...
	if (modeRuleContent && modeRuleContent.trim()) {
		if (usedRuleFile.includes(path.join(".novelweave", `rules-${mode}`))) {
	...
	// novelweave_change start: rule toggles
