...
export async function newTaskTool(
	...
	try {
		if (block.partial) {
			...
			// novelweave_change start: Switch to the desired mode BEFORE creating the task
			...
			if (!newTask) {
				await provider.handleModeSwitch(task.pausedModeSlug) // novelweave_change: if task creation failed, switch back to the parent's mode
