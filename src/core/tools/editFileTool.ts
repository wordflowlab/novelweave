// novelweave_change: Fast Apply -- file added
...
async function applyFastApplyEdit(
	...
	try {
		...
		const kiloTesterSuppressUntil = state.apiConfiguration.novelweaveTesterWarningsDisabledUntil
		const kiloTesterSuppress =
			kiloTesterSuppressUntil && kiloTesterSuppressUntil > Date.now() ? { [X_NOVELWEAVE_TESTER]: "SUPPRESS" } : {}
		...
		const client = new OpenAI({
			...
			defaultHeaders: {
				...
				...(morphConfig.kiloCodeOrganizationId
					? { [X_NOVELWEAVE_ORGANIZATIONID]: morphConfig.kiloCodeOrganizationId }
					: {}),
				...kiloTesterSuppress,
				[X_NOVELWEAVE_TASKID]: cline.taskId,
			},
		})
...
function getFastApplyConfiguration(state: ClineProviderState): FastApplyConfiguration {
	...
	// Priority 2: Use NovelWeave provider
	if (state.apiConfiguration?.apiProvider === "novelweave") {
		const token = state.apiConfiguration.novelweaveToken
		if (!token) {
			return { available: false, error: "No NovelWeave token available to use Fast Apply" }
		}
		return {
			available: true,
			apiKey: token,
			baseUrl: `${getNovelWeaveBaseUriFromToken(token)}/api/openrouter/`,
			model: selectedModel === "auto" ? "morph/morph-v3-large" : selectedModel, // Use selected model
			kiloCodeOrganizationId: state.apiConfiguration.novelweaveOrganizationId,
		}
	}
