...
import { NovelweaveOpenrouterHandler } from "../../api/providers/novelweave-openrouter"

// Hardcoded list of image generation models for now
const IMAGE_GENERATION_MODELS = ["google/gemini-2.5-flash-image"] // novelweave_change
	...
	try {
		if (!block.partial) {
			...
			const openRouterHandler = openRouterApiKey
				...
				: new NovelweaveOpenrouterHandler({
						novelweaveToken: kiloCodeApiKey,
						novelweaveOrganizationId:
							cline.apiConfiguration.apiProvider === "novelweave" &&
							cline.apiConfiguration.novelweaveToken === kiloCodeApiKey
								? cline.apiConfiguration.novelweaveOrganizationId
								: undefined,
					})
			...
			const result = await openRouterHandler.generateImage(
				...
				// novelweave_change end
				inputImageData,
				cline.taskId, // novelweave_change
			)
