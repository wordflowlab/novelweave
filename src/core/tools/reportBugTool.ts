...
export async function reportBugTool(
	...
	try {
		if (block.partial) {
			...
			const novelweaveVersion =
				vscode.extensions.getExtension("novelweave.novelweave")?.packageJSON.version || "Unknown"
			...
			const bugReportData = JSON.stringify({
				...
				novelweave_version: novelweaveVersion,
			})
			...
			if (text || images?.length) {
				...
				try {
					...
					await createAndOpenGitHubIssue("NovelWeave-Org", "novelweave", "bug_report.yml", params)
				} catch (error) {
