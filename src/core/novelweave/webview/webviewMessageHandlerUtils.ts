...
const resendMessageSequence = async (
	...
	if (!currentCline || currentCline.taskId !== taskId) {
		...
		vscode.window.showErrorMessage(t("novelweave:userFeedback.message_update_failed"))
		return false
	}
...
export const fetchNovelweaveNotificationsHandler = async (provider: ClineProvider) => {
	try {
		const { apiConfiguration } = await provider.getState()
		const novelweaveToken = apiConfiguration?.novelweaveToken

		if (!novelweaveToken || apiConfiguration?.apiProvider !== "novelweave") {
			provider.postMessageToWebview({
				type: "novelweaveNotificationsResponse",
				notifications: [],
			})
			return
		}
		...
		provider.log(`Error fetching Novelweave notifications: ${error.message}`)
		provider.postMessageToWebview({
			type: "novelweaveNotificationsResponse",
			notifications: [],
		})
	...
	try {
		...
		if (revert) {
			...
			if (checkpointMessage && checkpointMessage.text) {
				...
				vscode.window.showErrorMessage(t("novelweave:userFeedback.no_checkpoint_found"))
			}
		}
		...
		if (success) {
			vscode.window.showInformationMessage(t("novelweave:userFeedback.message_updated"))
		}
	} catch (error) {
