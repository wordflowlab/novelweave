...
type ClaudeCodeOptions = {
	systemPrompt: string
	systemPromptFile?: string // novelweave_change
...
async function generateTempSystemPrompt(options: ClaudeCodeOptions): Promise<string | undefined> {
	...
	const tempFilePath = path.join(os.tmpdir(), `novelweave-system-prompt-${uniqueId}.txt`)
...
export async function* runClaudeCode(
	...
	const systemPromptFile = await generateTempSystemPrompt(options) // novelweave_change
	const claudePath = options.path || "claude"
	let process

	try {
		process = runProcess({ ...options, systemPromptFile }) // novelweave_change
	} catch (error: any) {
...
function runProcess({
	...
	// const isWindows = os.platform() === "win32" novelweave_change
	...
	const stdinData = JSON.stringify(messages) // novelweave_change
