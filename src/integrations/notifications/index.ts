...
async function showMacOSNotification(options: NotificationOptions): Promise<void> {
	...
	try {
		...
		// Add NovelWeave logo
		const extensionUri = vscode.extensions.getExtension(`novelweave.novelweave`)!.extensionUri
		const iconPath = vscode.Uri.joinPath(extensionUri, "assets", "icons", "novelweave.png").fsPath
...
"@
    ...
    [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier("NovelWeave").Show($toast)
    `
...
export async function showSystemNotification(options: NotificationOptions): Promise<void> {
	try {
		const { title = "NovelWeave", message } = options
