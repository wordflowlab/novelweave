...
export abstract class BaseTelemetryClient implements TelemetryClient {
	...
	protected async getEventProperties(event: TelemetryEvent): Promise<TelemetryEvent["properties"]> {
		...
		if (provider) {
			try {
				...
				providerProperties.exception = error instanceof Error ? error.stack || error.message : String(error) // novelweave_change
			}
		}
	...
	public updateIdentity(_novelweaveToken: string): Promise<void> {
