...
export class PostHogTelemetryClient extends BaseTelemetryClient {
		...
		super(
			{
				...
				events: [
					...
					// TelemetryEventName.LLM_COMPLETION // novelweave_change
				],
			},
			debug,
		)

		this.client = new PostHog(process.env.NOVELWEAVE_POSTHOG_API_KEY || "", {
			host: "https://us.i.posthog.com",
			disableGeoip: false, // novelweave_change
		})
	}
	...
	private counter = 0
	private novelweaveToken = ""

	public override async updateIdentity(novelweaveToken: string) {
		if (novelweaveToken === this.novelweaveToken) {
		...
		try {
			...
			if (id === this.counter) {
				...
				this.novelweaveToken = novelweaveToken
