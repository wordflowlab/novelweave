...
package ai.novelweave.jetbrains.plugin
...
import ai.novelweave.jetbrains.core.ExtensionProcessManager
import ai.novelweave.jetbrains.core.ExtensionSocketServer
import ai.novelweave.jetbrains.core.ServiceProxyRegistry
import ai.novelweave.jetbrains.webview.WebViewManager
import ai.novelweave.jetbrains.workspace.WorkspaceFileChangeManager
...
import ai.novelweave.jetbrains.core.*
import ai.novelweave.jetbrains.util.ExtensionUtils
import ai.novelweave.jetbrains.util.PluginConstants
import ai.novelweave.jetbrains.util.PluginResourceUtil
import java.io.File
...
class WecoderPluginService(private var currentProject: Project) : Disposable {
    ...
    companion object {
        // Debug mode switch
        @Volatile
        private var DEBUG_TYPE: DEBUG_MODE = ai.novelweave.jetbrains.plugin.DEBUG_MODE.NONE
        ...
        init {
            try {
                ...
                val configStream: InputStream? = WecoderPluginService::class.java.getResourceAsStream("/ai/novelweave/jetbrains/plugin/config/plugin.properties")
...
//                ServiceProxyRegistry.getInstance().initialize()
                ...
                if (DEBUG_TYPE == ai.novelweave.jetbrains.plugin.DEBUG_MODE.ALL) {
    ...
    private fun cleanup() {
        ...
        try {
            if (DEBUG_TYPE == ai.novelweave.jetbrains.plugin.DEBUG_MODE.NONE) {
