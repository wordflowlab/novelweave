...
package ai.novelweave.jetbrains.editor
...
import ai.novelweave.jetbrains.commands.CommandRegistry
import ai.novelweave.jetbrains.commands.ICommand
import ai.novelweave.jetbrains.util.URI
import ai.novelweave.jetbrains.util.URIComponents
import java.io.File
    ...
    suspend fun vscode_changes(title: String, changes: List<List<Map<String, Any?>>>): Any? {
        ...
        try {
            ...
            for ((index, changeTriplet) in changes.withIndex()) {
                ...
                val originalFileUri = ai.novelweave.jetbrains.editor.createURI(changeTriplet[0])
                val beforeContentUri = ai.novelweave.jetbrains.editor.createURI(changeTriplet[1])
                val afterContentUri = ai.novelweave.jetbrains.editor.createURI(changeTriplet[2])
